...
if (ptr->flags&DIRTY1) {
   ptr->flags &= (~DIRTY1);
   Seek(ptr->lfp, ptr->block1_start*2, OFFSET_BEGINNING);
   if (Write(ptr->lfp, (char *)ptr->blockl1, BLOCK_SIZE*2)!=BLOCK_SIZE*2)
      return(WRITE_ERROR);
   if (ptr->type==STEREO) {
      Seek(ptr->rfp, ptr->block1_start*2, OFFSET_BEGINNING);
...
Seek(ptr->lfp, ptr->block1_start*2, OFFSET_BEGINING);
...
Seek(ptr->lfp, ptr->block2_start*2, OFFSET_BEGINNING);
len = (ptr->length - ptr->block1_start)*2;
...
if (ptr->type==STEREO) {
   Seek(ptr->rfp, ptr->block2_start*2, OFFSET_BEGINNING);
...
quick_write(ptr,data,len)
char *data;
struct Disk_Samp *ptr;
int len;
{
ptr->length += len;
if (Write(ptr->lfp, data, len*2)!=len*2)
   return(WRITE_ERROR);
else
   return(NO_ERROR);
}
...
struct Disk_Samp *ptr;
{
if (Read(ptr->lfp, data, BIG_BLOCK_SIZE*2)!= BIG_BLOCK_SIZE*2)
   return(READ_ERROR);
else
   return(NO_ERROR);
}
...
ptr->lfp = Open(ptr->name, MODE_OLDFILE);
if (ptr->lfp==-1)
   return(CANT_OPEN_FILE);
ptr->block1_start = 0;
ptr->block2_start = 0;
