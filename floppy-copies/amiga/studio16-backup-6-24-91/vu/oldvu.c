...
if(!(VU->count--))
	...
	VU->count=30;
			...
			SetOPen(wind->RPort, BLACK);
			RectFill(wind->RPort,VU->x+d,DIGITstart-1,VU->x+d+6,DIGITstart+1);
			VU->PEEK=VU->PPEEK; VU->PPEEK=0;
			d=(VU->PEEK<<3)+1;
			if(VU->PEEK>14) {SetAPen(wind->RPort, RED);SetOPen(wind->RPort, RED);}
			else {SetAPen(wind->RPort, 5);SetOPen(wind->RPort, 5);}
			RectFill(wind->RPort,VU->x+d,DIGITstart-1,VU->x+d+6,DIGITstart+1);
	}
...
if(l!=HLL)
	...
	if (l>HLL)
		...
		while(l!=HLL)
			...
			d=(HLL<<3)+1;
			if(HLL>14) {SetAPen(wind->RPort, RED);SetOPen(wind->RPort, RED);}
			else {SetAPen(wind->RPort, 5);SetOPen(wind->RPort, 5);}
			RectFill(wind->RPort,VU->x+d,DIGITstart-1,VU->x+d+6,DIGITstart+1);
			}
		}
