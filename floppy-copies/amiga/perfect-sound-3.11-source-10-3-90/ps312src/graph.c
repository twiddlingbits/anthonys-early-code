...
for (x=LGRAPH_X; x < LGRAPH_X+num_samples*6; x=x+6) {
   if (index >=0 && index <len) {
      i=*(samp[cur].lmem+index);
      if (i > 0)
         endy = y-(i*(LGRAPH_H/2))/127;
      else
         endy = y-(i*(LGRAPH_H/2-1))/128;
      if (index==start_mark) {
         Move (&screen->RastPort, x+2, LGRAPH_Y);
         Draw (&screen->RastPort, x+2, LGRAPH_Y+LGRAPH_H-1);
         }
      for (i=0; i < 5; i++) {
...
for (x=RGRAPH_X; x < RGRAPH_X+num_samples*6; x=x+6) {
   if (index >=0 && index <len) {
      i=*(samp[cur].lmem+index);
      if (i > 0)
         endy = y-(i*(RGRAPH_H/2))/127;
      else
         endy = y-(i*(RGRAPH_H/2-1))/128;
      if (index==end_mark) {
         Move (&screen->RastPort, x+2, RGRAPH_Y);
         Draw (&screen->RastPort, x+2, RGRAPH_Y+RGRAPH_H-1);
         }
      for (i=0; i < 5; i++) {
         Move (&screen->RastPort, x+i, y);
         Draw (&screen->RastPort, x+i, endy);
...
static char lastpt[500];

SetAPen(rec_rp,0);
WritePixel(rec_rp,xpos,lastpt[xpos-REC_X]/REC_SCALE+REC_ZERO);
lastpt[xpos-REC_X]=pt;
SetAPen(rec_rp,1);
WritePixel(rec_rp,xpos,pt/REC_SCALE+REC_ZERO);
xpos++;
if (xpos > REC_X2) xpos=REC_X;
