...
LastPlayRate=hand_state->sampling_rate[0];
...
if (samp)
	...
	if (((struct droplist *)(IRateBut.gad_ptr))->l->current==0) ;
	else region[x].parms.rate=GlobSampRate;

	LastPlayRate=region[x].parms.rate;
	...
	if ((as[x]=(struct ActiveSample *)ASOpen(&region[x], AS_PLAY|AS_AUTO_DATA|AS_AUTO_CLOSE|AS_AUTO_FREECHAN|ASFLAG))!=0)
		...
		if (as[x]->key==0)
			{
			Playing++; /* Required Because ASCLOSE SEnds a PLAYBACK_DONE message */
			ASClose(as[x]);
...
if (CreateSample(sn, 0, &samp)==NO_ERROR)
	...
	if (((struct droplist *)(IRateBut.gad_ptr))->l->current==0) region[4].parms.rate=LastPlayRate;
