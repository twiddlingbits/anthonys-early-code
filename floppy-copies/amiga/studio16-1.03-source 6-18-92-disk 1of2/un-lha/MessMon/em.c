...
long _priority = 60;			/* The priority to run us at              */
...
mod->state->pref_name="Mess Mon Prefs";
...
if ((wind = OpenNewWind(&NewWind,mod->state,PREFERENCES,NULL,NULL))==0)  {telluser("Event Monitor:","Can not open window!",0);Quit();}
...
Disable();
for (i=0; i < NumOfID; i++)
	if (i!=se_info_hilo && i!=se_info_hilo && i!=se_cmd_select_module && i!=se_cmd_kill_module)
