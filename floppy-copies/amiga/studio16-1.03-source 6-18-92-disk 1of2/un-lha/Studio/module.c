...
FUNCTION
	...
	when loading or saving sample files.	You pass a pointer
...
NAME	
	LoadSamp -
FUNCTION
	Creates a new studio16 "Disk_Samp" from an AmigaDos file.
...
for (node=StudioBase->file_filters.lh_Head; node->ln_Succ; node = node->ln_Succ) {
	...
	if (ff->flags&FF_LOAD_FILTER) {
		if ((error=((*ff->func)(infp, wsp)))!=STDERR_UNKNOWN_FILE_FORMAT) {
			break;
			}
		}
	}
...
FUNCTION
	Saves a sample to the specified path/file using the format
	as specified by ff (a filefilter pointer.)  You should scan
	StudioBase->->file_filters to determine which fileformat
	you want to save in.  For example, file filters are included
	for 8SVX or AIFF.
INPUTS
	path	directory to save to
	fname	filename to save to (path+fname gives complete path)
	samp	Disk_Samp to save
	ff		FileFilter to use when saving
RESULTS
	Standard error code
##############################################################*/
...
FUNCTION
	...
	as a working file.
INPUTS
	path,name source path if "!saveing", destination if saving
	samp	Disk_Samp to get name & version from
	saving TRUE if renaming from audio:,
		    FALSE if renaming into audio:
RESULTS
	Standard error code
##############################################################*/
