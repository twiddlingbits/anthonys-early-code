...
/* V0.52 V1.0 Compatabilty fix */ if (PS->region.parms.volume==0x8000) PS->region.parms.volume=3200;
	...
	/***************************************/
		{
		if (tkey=(struct ChanKey *)event->arg1)
			{
...
/* * */
	...
	/***************************************/
		...
		if (event->arg1==(int)hand)
			{
			x=event->arg2-state->input_gain[0];
...
for(x=0;x<4;x++)
	{
	SendW(ChanVol+((x+1)<<8));
	SendW(DBs[MIN(state->vol_play[x],3393)]);
	}

SendW(ChanVol);
SendW(DBs[MIN(state->vol_in[0],3393)]);

SendW((5<<8)+ChanVol);
SendW(DBs[MIN(state->vol_out[0],3393)]);

SendW(SetFreq+FindFreq(state->filter[0]));
BroadcastEventParmsLessOne(se_cmd_set_cutoff_freq,mod,hand,state->filter[0], 0, 0, 0,0);

rate=(HALF_CLK/state->sampling_rate[0])-1;
SendW(SetRate);
SendW(rate);
BroadcastEventParmsLessOne(se_cmd_set_sampling_rate,mod,hand,state->sampling_rate[0],0, 0, 0,0);

SendW(GainDown+(101<<8));
if(state->input_gain[0]) SendW(GainUp+(state->input_gain[0]<<8));
BroadcastEventParmsLessOne(se_cmd_set_input_gain,mod,hand,state->input_gain[0], 0, 0, 0, 0);
}
/********************************************************************/
/********************************************************************/
