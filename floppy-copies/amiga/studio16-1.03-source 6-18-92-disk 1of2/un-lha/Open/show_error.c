...
switch (err) {
	...
	case STDERR_SINGLE_RESIDENT:
		telluser("Cant add Module","Only one of these modules may be resident.",0);
	break;

	case STDERR_NO_TAG:
		telluser("ERROR: No Tag","",0);
	break;

	case STDERR_CANT_CREATE_SAMPLE:
		telluser("Can not create sample.","",0);
	break;

	case STDERR_CANT_OPEN_SAMPLE:
		telluser("Can not open sample","",0);
	break;
	...
	case STDERR_DATA_NOT_AVAILABLE_YET:
		telluser("Data not available yet.","",0);
	break;

	case STDERR_DATA_EOF:
		telluser("End of file","",0);
	break;
	...
	case STDERR_NO_MORE_CLIPS:
		telluser("Can't perform edit.  This sample has too many","edits in it. Use 'Compress'.",0);
	break;
