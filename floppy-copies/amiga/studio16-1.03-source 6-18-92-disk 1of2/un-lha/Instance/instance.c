...
static struct NewList instancelist =
			{NULL,10,16,0,0,LIST_COOL|LIST_SINGLESELECT|LIST_SORTED|LIST_USEKEYS|MANUAL_XOR,0,0,0,0,0,0,0,0,0};
...
while ((event=(struct GadMsg *)GetMsg(gadlibport))!=0) {
	if (event->g==instancelist.gad_ptr) {
		...
		if (strncmp(StudioBase->screen->Title,"Studio 16",9))
			telluser("Instance:","This function Currently Disabled in B&P ",NULL);
		else
			{
			new_pointer(PTR_BUSY);
