...
/*      printf("%c",line[cy].letters[i]);
  */    }
/*   printf("\n");  */
   line[cy].letters[cx] = letter;
   line[cy].pos[cx] = line[cy].pos[cx-1] + twin->letw;
   if (cx == 0) {      /* this whole if section is pretty damn stopgap! */
      i=0;
      while(i < line[cy].l + 2)
         line[cy].pos[i] = (twin->letw * i++) + 4;
      }
   line[cy].l++;
   }
/* if it looks like the line is too long for the screen... */
