...
int len,originallen,blanklen,off2,off3,*f;
struct textwin twin;
struct RastPort *rp;
struct Menu *Tit1,*textedmenus();
int jumpahead,tempstart,i,j,k,l,cflag,insert;
int cutlen,y,charh,charw,curline,curlalt,tx,ty;
LONG mclass;
...
static char fname[50],fpath[50],temp1[2],temp2[3];
char *cutstring;
int cc1,cc2;
static struct BitMap holdbm;
static struct IOStdReq ioreq;
static struct InputEvent ievent = {NULL,IECLASS_RAWKEY,0,0,0};
static int owinx,owiny,owinw,owinh,otextlength,ooffset;
static int ocursorx,ocursory;
...
if (otextlength == strlen(string)) {
   off = ooffset;
   cursorx = ocursorx;
   cursory = ocursory;
   }

if (string != 0 && owinw + owinh != 0) {
   wx = owinx;
   wy = owiny;
   ww = owinw;
   wh = owinh;
...
*/
      ...
      if (mclass == RAWKEY && mcode[0] < 128) {
         ...
         if (temp1[0] == 7 ||temp1[0] == 8 ||temp1[0] == 9 ||
           ...
           goto jumppast;
         if (((mqual & 207) == 0 || (mqual & 200) == 0) && k > 0) {
            ...
            if(i == 1) {
               ...
               i = curblock->start[curline+cursory];
               if (curline+cursory > 49)
                  i = curblock->next->start[curline+cursory-50];
               ...
               j = curblock->start[curline+cursory];
               if (curline+cursory > 49)
                  j = curblock->next->start[curline+cursory-50];

               if (j > i && cursory > 0) {    /* if back-wrap has happened */
                  cursorx = cursorx - (j-i);
                  if (cursorx < 0) {
                     cursorx = cursorx + line[--cursory].l;
                     }
                  }
               }
            cursorx++;
...
*/
      ...
      if (mclass == RAWKEY && mcode[0] == 194) {   /* tab key */
            goobleedflag=1;
         mcode[0]=0;
         temp1[0]='@';
         goto cheapjump1;
      ...
      if (mclass == RAWKEY && mcode[0] == 86 && mqual == 32793) {
         mcode[0]=0;
         mn1 = -1;
         mqual = 0;
         movmem((char *)bohaha(),string2,strlen((char *)bohaha()) + 1);
         goto bhhjump;
         bhhret:
         }
      ...
      if (mclass == GADGETUP) {
         ...
         if (maddress != (APTR) gooblegadget[2]) {
            i = curblock->start[curline] / PROPSPACE;
            ModifyProp(gooblegadget[2],win,NULL,goobleprop->Flags,
                       goobleprop->HorizPot,
            (0xffff*(i))/(len/PROPSPACE+1), goobleprop->HorizBody,
         ...
         while (mcode[0] != MENUNULL) {
            ...
            if (mn1 == 0 && mn2 == 1) {           /* load text file */
               ...
               if (i != NULL) {
                  fwhole[0] = 0;
                  sprintf(temp2,"");
                  if (fpath[strlen(fpath)-1] != ':' && strlen(fpath) > 2)
                     strncat(temp2,"/",2);
                  strncat(&fwhole[0],&fpath[0],49);
                  strncat(&fwhole[0],&temp2[0],49);
                  strncat(&fwhole[0],&fname[0],49);
                  newpointer(screen,BUSY);
                  if (mn3 == 0)
                     i = textedloadtext(&originallen,&string2,&fwhole[0],0);
                  else 
                     i = textedloadtext(&cutlen,&cutstring,&fwhole[0],1);
                  newpointer(screen,NORMAL);
                  if (i == OUTOFMEM)
            ...
            if (mn1 == 0 && mn2 == 2) {           /* save text file */
               ...
               if (i != NULL) {
                  ...
                  if (mn3 == 0)
                     ...
                     i = textedsavetext(strlen(cutstring),cutstring,
                         &fwhole[0]);
                  newpointer(screen,NORMAL);
                  if (i == FILENOTFOUND)
            ...
            if (mn1 == 0 && mn2 == 6) {           /* quickie end */
               *status = AOK;
               ...
               if (*status !=BADCD)
                  ...
                  if (strlen(string2) == 1 || goobleedflag == 0)
                     *status = JUSTQUIT;
                  else
                     *status = AOK;
                  i = TRUE;
               ...
               if (i == TRUE) {
                  if (string != 0) {
                     if(curline==49)
                        ooffset = curblock->start[curline];
                     else
                        ooffset = curblock->start[curline+1];
                     otextlength = strlen(string2);
                     owinx = win->LeftEdge;
                     owiny = win->TopEdge;
                     owinw = win->Width;
                     owinh = win->Height;
            ...
            if (mn1 == 1 && mn2 == 4 && searchstr[0] != 0) {       /* replace */
               ...
               do {
                  ...
                  if (jumpahead != -1) {
                     ...
                     tempstart = tempstart+strlen(repstr);
                     }
                  }while (jumpahead != -1);
               if (off3 != tempstart) {
                  makewin(&twin,string2,curline,curblock,&len);
               ...
               do {
                  ...
                  if (jumpahead != -1) {
                     ...
                     *status = fillandfind(off,string2,&twin,&curblock,
                     ...
                     i = replacereq(screen,win);
                     if (i == AOK) {
                        replace(string2,tempstart,searchstr,repstr,&len);
                        tempstart = tempstart+strlen(repstr);
                        }
                     else {
                        if (i == BADIFF) goto queryjump;
                        else
                           tempstart++;
                        }
                     }
            ...
            if (mn1 == 1 && mn2 == 7) {       /* cut */
               setcursor(&twin,cursorx,cursory);
               newpointer(screen,CROP);
               if(curline==49)
                  off = curblock->start[curline];
               else
                  off = curblock->start[curline+1];
               i = draglines (&twin,&cc1,&cc2,&curline,&curblock,&ld,&curblock2,
                          &curblock3,string2,&off2,len,&holdbm);
               if (i == AOK) {
               if (cutstring != 0)
                  FreeMem(cutstring,cutlen);
               i = cc1 - cc2;
               if (i < 0)
                  i = cc2 - cc1;
            ...
            if (mn1 == 1 && mn2 == 9) {       /* paste */
               if (cutstring == NULL)
                  ...
                  pasteflag = 1;
                  newpointer(screen,PASTE);
                  }
               }

            if (mn1 == 1 && mn2 == 10) {       /* erase */
                  newpointer(screen,TRASHCAN);
               if(curline==49)
                  off = curblock->start[curline];
               else
                  off = curblock->start[curline+1];
               ...
               if (i == AOK) {
                  ...
                  if (i == TRUE) {
                     ...
                     curblock = &ld;
                     curline = 0;
/*                     *status = fillandfind(cc1,string2,&twin,&curblock,
                               &curline,&lastblock,0);
*/                     *status = fillandfind(off,string2,&twin,&curblock,
                               &curline,&lastblock,0);
                     clearwindow(&twin);
