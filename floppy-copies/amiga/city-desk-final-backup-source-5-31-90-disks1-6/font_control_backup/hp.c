...
/** Gerenate .FNT files for Q&A Write.  City desk is cd_hp.c ***/
...
int w_fh, r_fh;
int append;
static unsigned char widthtable[256];
...
setmem(widthtable,256,0);
setmem(header,256,0);

/* lf.widthtable[32] = 1; */

while (process_esc(r_fh, widthtable, header));
close (r_fh);

if (widthtable[32]==0)
   widthtable[32] = widthtable[0x69];  /** use width of 'i' **/

header[0]=1;
header[0xf2]=0x2c;
header[0xf3]=0x01;
header[0xf5]=0x01;
strcpy(&header[1], cd_name);
sprintf(&header[0xea], "P");
strncpy(&header[41], fontname,3);
/* printf("style=%x\n",lf.style); */
/* sprintf(lf.fonton,"\033(%dX",font_id); */
write(w_fh,header,256);
write(w_fh,"",1);
write(w_fh,widthtable,255);
close (w_fh);
stat("Font Created.");
}


process_esc(fh, widthtable, header)
...
/* printf("height in pts %f\n",fpts); */
 ...
 /* printf("[&l%dO[(%s[(s%dP%2.2fH%2.2fV%dS%dB%dT",
  ...
  sprintf(&header[0x54],"\033&l%dO\033(%s\033(s%dp%2.2fh%2.2fv%ds%db%dt",
       buf[13],sym[symnum],buf[14],fpitch,fpts,buf[24],buf[25],buf[26]);
