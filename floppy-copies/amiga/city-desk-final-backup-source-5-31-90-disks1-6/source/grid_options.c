...
static struct Gadget independent = {
   &up,   /* next gadget */
...
static struct IntuiText IText3 = {
   ...
   NULL,   /* font pointer or NULL for default */
   "Independent X/Y Grid",   /* pointer to text */
...
static char *val_list[LISTSIZE] = {"1","4","8","16","32","36","48","72","100"};
char *buf;
...
sprintf(x_valSIBuff,"%d",(x_gridsize*72+xinch/2)/xinch);
sprintf(y_valSIBuff,"%d",(y_gridsize*72+yinch/2)/yinch);

buf = x_valSIBuff;         /*** default independent selection is x ***/
...
while (TRUE) {
   ...
   while( (message=GetMsg(window->UserPort))!=0) {
      ...
      if (class == GADGETDOWN && g==&x_val) {
         ...
         CloseWindow(window);
         i=convert_string(x_valSIBuff,NUM_COORD_X,NUM_DEF_POINT);
         if (i>1 && i < 4896)
            x_gridsize = i;
         i=convert_string(y_valSIBuff,NUM_COORD_Y,NUM_DEF_POINT);
         if (i>1 && i < 4896)
            y_gridsize = i;
