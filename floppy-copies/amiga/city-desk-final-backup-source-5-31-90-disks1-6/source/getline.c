...
if (gotobj==0) {
   ...
   rule_width = ((struct rule *)(obj->data))->width;
   }
...
/*         obj->w=max(40,defaultrule.width); */
      ...
      obj->type=RULE;
      rp->LinePtrn=0xFFFF;
      displayobject(page[pg].view,obj);
      return(AOK);
      }
   ...
   if (lm) { /** don't update if lots of messages qued up **/
      if (type==HORIZ) {   /** reset w & h incase use changed line dir */
         obj->h=rule_width;
         }
      else {
         obj->w=rule_width;
         }
      show_coord(obj, newx, newy, 0, 0, endx-startx, endy-starty);
      }
   }
}
