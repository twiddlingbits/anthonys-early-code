...
if (page->pn==CB) {
   ...
   page->view->mag = default_mag;
   page->view->window->MaxWidth=
            min(std_width,XBORDER_AREA+ctosx(page->pw*default_mag));
   page->view->window->MaxHeight=
            min(screensize,YBORDER_AREA+ctosy(page->ph*default_mag));
   }

page->view->h=page->ph/page->view->mag;
page->view->w=page->pw/page->view->mag;

if (default_mag!=1 && page->pn!=CB)
   correct_sliders(page->pn);
