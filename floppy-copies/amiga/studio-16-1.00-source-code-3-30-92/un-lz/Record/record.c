...
if (CreateSample(fn, 0, &samp)==NO_ERROR)
	...
	else {CloseSample(samp);if (as) ASStop(as); as=NULL;telluser("Record:","Can not open record sample, check Work: path",NOTICE_RETURN_NOW);but[2].gad_ptr->flags&=~BUTN_HIT;refresh_gadget(but[2].gad_ptr);return;}
	}
...
while ((event=(struct StudioEvent *)GetMsg(mod->notifyme))!=0)
	...
	if (event->type == se_info_block_recorded && as)
		...
		TempSize=ASPos(ps)*2;	/** Get Record position **/
		RecordStats();
		}
...
if (Rwind)
	...
	SetBPen(Rwind->RPort,StudioBase->defaults.colors.gadget_window.hitbox_unselected);
	SetDrMd(Rwind->RPort, JAM2);

	if (flag) 
		if(!(StudioBase->defaults.flags&SBD_SMPTE_ON)) {istc=0xFFFFFFFF;sprintf(str,"StartTime NA",stc->hours,stc->minutes,stc->seconds,stc->frames);flag=0;Move(Rwind->RPort, 5,38);Text(Rwind->RPort, str,strlen(str));}
		else {sprintf(str,"StartTime %02d:%02d:%02d:%02d",stc->hours,stc->minutes,stc->seconds,stc->frames);flag=0;Move(Rwind->RPort, 5,38);Text(Rwind->RPort, str,strlen(str));}

	if (istc==0) {flag=1;}
	if (flag) {sprintf(str,"Name:%-16s",fn);Move(Rwind->RPort, 5,20);Text(Rwind->RPort, str,strlen(str));}


	q=(TempSize)/(1024*1024);
	w=(((TempSize-(q*1024*1024))/1024)*100)/1024;
