...
g[1].GadgetRender=(APTR)&image;
g[1].Activation=FOLLOWMOUSE|GADGIMMEDIATE;
g[1].Flags=GADGHNONE;

/* prop structure */
hbdy=0xFFFF/(imax-imin);
if (hbdy*(imax-imin)==0xFFFF) hbdy--;
prop.Flags=AUTOKNOB|FREEHORIZ;
prop.HorizPot=(SHORT)(((ULONG)(ival-imin)*(ULONG)0xFFFF)/(ULONG)(imax-imin))+hbdy;
prop.VertPot=0;
prop.HorizBody=hbdy;
...
if (class==MOUSEMOVE) {
   ival=(ULONG)(prop.HorizPot*(ULONG)(imax-imin))/(ULONG)0xFFFF+imin;
   stci_d(ascii,ival,80);
   s.LongInt=ival;
...
if (class==GADGETUP && address==&g[2]) {    /* new integer */
   ...
   hor=(SHORT)(((ULONG)(ival-imin)*(ULONG)0xFFFF)/(ULONG)(imax-imin))+hbdy;
