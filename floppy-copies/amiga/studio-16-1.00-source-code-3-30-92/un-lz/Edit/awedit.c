...
/*if (error!=NO_ERROR) kprintf("PASTE ERROR!!\n");*/
return(error);
}
...
struct EditInstance *ei;
{
/*kprintf("gonna FreeGraphBuffer! size %04x%04x\n",ei->gb_size);*/
if (ei->graph_buffer) {
...
struct EditInstance *ei;
{
/*kprintf("Gonna Free Render buffer! size %04x%04x\n", ei->plane_size);*/
if (ei->bm.Planes[0]) {
	FreeMem(ei->bm.Planes[0], ei->plane_size);
	ei->bm.Planes[0]=0;
	}
/*kprintf("Did Free Render buffer!\n");*/
}
/************************************************************/
...
/*kprintf("x1 %04x%04x x2 %04x%04x width %04x%04x\n",x1-GRAPH_X,x2-GRAPH_X,ei->graph_width);*/
...
if (ei->mark_drawn) {
	   if (x1 < ei->old_x1) {
/*kprintf("AAA x1 %04x%04x old %04x%04x\n",x1,ei->old_x1);*/
...
/*kprintf("BBB  x1 %04x%04x old %04x%04x\n",x1,max(ei->old_x1,GRAPH_X));*/
	   ...
	   if (x2 > ei->old_x2) {
/*kprintf("CCC x2 %04x%04x old %04x%04x\n",x2,ei->old_x2);*/
...
if (WaitingForTimer) {
	...
	if (GetMsg(timerport))	/** Cancel any pending timer events **/
		kprintf("why is this here?\n");
	}
}
