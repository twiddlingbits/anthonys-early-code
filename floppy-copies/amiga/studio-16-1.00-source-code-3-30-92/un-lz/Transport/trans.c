...
if (!Playing && !as[4])
	...
	if (RECORD) as[4]=RecSam(RecFileName);
...
if (b->flags&BUTN_HIT) 
	...
	RecordStats();
/*	SetAPen(wind->RPort,StudioBase->defaults.colors.gadget_window.bottomright_3d);
	Move(wind->RPort,px1+12,py4+8);Text(wind->RPort,"RECORD CHANNEL",14);*/
	}
else
	{
	if (as[4] && RECORD==TRUE) {ASStop(as[4]); hide_gadget(RecBut.gad_ptr);}
...
if (RECORD)
	...
	w=(((TempSize-(q*1024*1024))/1024)*1000)/1024;
