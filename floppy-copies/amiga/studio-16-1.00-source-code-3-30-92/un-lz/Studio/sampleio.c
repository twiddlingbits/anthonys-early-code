...
FUNCTION
	...
	with Disk_Samp's, not files.  SampleRead undersands 
...
RESULTS
	the number of samples actualy read
##############################################################*/
...
/*sprintf(bug,"SampleRead index %d .start %d .end %d\n",sp->dclipindex, dclips[sp->dclipindex].start, dclips[sp->dclipindex].end);
dprintf(bug);*/
	readlen = min(length, dclips[sp->dclipindex].end-sp->fileindex+1);
	totalread+=SRead(sp->fp,buffer,readlen*2);
...
NAME	
	SampleWrite -
FUNCTION
	This is similars to a dos Write() function, except it deals
	with Disk_Samp's, not files.  SampleWrite undersands 
	DataClips inside a sample and deals with them.   You can
...
INPUTS
	...
	length number of samples to write (not bytes!) Make sure
			your buffer points to length*2 bytes!
RESULTS
	the number of samples actualy written.  If this is shorter
...
if (sp->dclipindex==0 && dclips[0].end==0) {
	return(SampleAppend(sp, buffer, length));
	}
		...
		if (sp->fileindex > dclips[sp->dclipindex].end) {
			...
			if (dclips[sp->dclipindex].end) {
				...
				if (dclips[sp->dclipindex].start==sp->disksamp->org_length) {
					return(totalwritten/2+SampleAppend(sp, buffer, length));
					}
...
NAME	
	SampleSeek -
FUNCTION
	This is similars to a dos Seek() function, except it deals
	...
	SampleSeek() seeks too probably won't match the position you
...
if (rpos==0 && mode==SS_OFFSET_CURRENT)	/** may speedy **/
	return(iamat);
...
/** are we at the last .end+1? **/

if (samp->dclips[i-1].end==samp->org_length-1 &&
	 editval==samp->org_length) {
	*fileindex=samp->org_length;
	*dclipindex=i-1;
	}
else {
	/** Initilize a new clip to zero length **/
...
FUNCTION
	...
	how big of a temporary buffer to allocate.
INPUTS
