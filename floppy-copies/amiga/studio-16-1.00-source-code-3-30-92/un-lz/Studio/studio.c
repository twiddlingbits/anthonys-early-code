...
/** for speed.  As such, they can't be access inside an inturrupt. **/
...
while (TRUE) {
	Wait(1<<studio_reply_port->mp_SigBit);
	while (e = (struct StudioEvent *)GetMsg(studio_reply_port)) {
		UseSendMsg(e);
		}
	}
...
if (fp) {
	...
	StudioBase->defaults.flags&=(~SBD_SMPTE_ON);
...
#define WINDCOLORS {MEAD,LITE,DARK,MEAD,DARK,ECL,ECL}
...
strcpy(StudioBase->defaults.backdrop_file,"s:Studio16Backdrop");
...
/** zoro all references to this handler! **/
...
INPUTS
	...
	flags			Search for a handler that matches these flags.
				   ...
				   set it to -1;
	num_bits	   Search for a handler that his the specified number
	...
	class_id		Search for this particular instance of a handler
					...
					two AD1012 cards, there would be two handler.
...
FUNCTION
	...
	CK_ID_ONLY		 This means that you don't want to actually
						 ...
						 channel.  For example, if you wan't to send
...
if (cn!=-1) {
	...
	for (i=0; i < ah->num_channels[chantype] ; i++)	{
		ChanBit=(1<<i);
		if ((ah->chans_in_use[chantype]&ChanBit)==0) {		/** Free chanel !! **/
			ah->chans_in_use[chantype] |= ChanBit;
...
FUNCTION
	Free's a previously allocated channel.
INPUTS
...
FUNCTION
	Given a module name, this function will load it's code
...
Execute(path,0L,nil);		/** zero nil is sortof ok if Open failed **/
if (nil) {
	Close(nil);
	}

/** Now wait for the module to register itself.  We are asuming **/
...
/** to search for the handler and asign in automaticly **/
