...
while (TOflag==1)
	...
	if(tcflag==1)
		...
		{
		if (ltc!=tc) {tcflag=1;StudioBase->defaults.flags|=SBD_SMPTE_ON;}
		}
	ltc=tc;
	}
...
if (ss) {
	...
	if (SmpteSourceCount==0 && TOflag==0) {STtask=(struct Task *)CreateTask("STUDIO_SMPTE_TIMEOUT",0,SmpteTimeout,2000);TOflag=1;}
	SmpteSourceCount++;
	Enable();
