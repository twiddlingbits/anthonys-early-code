...
wp->ActiveClrs=&AWindClr;
wp->InactiveClrs=&IWindClr;
nwind->Title=NULL;
...
else {clrs=wp->ActiveClrs;}

LockGadLibRender();
SetDrMd(wind->RPort,JAM1);
SetAPen(wind->RPort,wp->InactiveClrs->hitbox_unselected);
...
if (x==0 || (wind->Flags&WINDOWACTIVE)!=(wp->Flags2&WINDOWACTIVE))
	...
	if (wind->Flags&WINDOWACTIVE) 
		{
		clrs=wp->ActiveClrs;gclrs=&Acolors;
		wp->Flags2|=WINDOWACTIVE;
		GlobActivWind=wind;
		if (StudioBase->defaults.flags&WIND_POPTOFRONT) WindowToFront(wind);
		}
	else {FLAG=1;clrs=wp->InactiveClrs;gclrs=&Icolors;wp->Flags2&=~WINDOWACTIVE;if(GlobActivWind==wind) GlobActivWind=NULL;}
	...
	if (wp->Flags&WINDOWCLOSE) 
		{
		if (StudioBase->defaults.flags&WIND_SHORTTITLEBAR && FLAG==1)
			{
	...
	if (wp->Flags&WINDOWDEPTH)
		...
		if (StudioBase->defaults.flags&WIND_SHORTTITLEBAR && FLAG==1)
			...
			if (wp->DepthButton.gad_ptr->flags&BUTN_HIT) 
				{
				draw_indent(wind->RPort,wind->Width-20,2,wind->Width-9,6,clrs->topleft_3d,clrs->bottomright_3d);
				draw_indent(wind->RPort,wind->Width-16,4,wind->Width-5,8,clrs->topleft_3d,clrs->bottomright_3d);
				SetAPen(wind->RPort,gclrs->button.hitbox_selected);
				}
			else 
				{
	...
	if (wp->Flags&WINDOWSIZING)
		...
		if (StudioBase->defaults.flags&WIND_SHORTTITLEBAR && FLAG==1)
			...
			if (wp->DSizeButton.gad_ptr->flags&BUTN_HIT) 
				{
				draw_indent(wind->RPort,R-24+4,2,R-24+19,8 ,clrs->topleft_3d    ,clrs->bottomright_3d);
				draw_indent(wind->RPort,R-24+12,5,R-24+17,7,clrs->bottomright_3d,clrs->topleft_3d    );
				}
			else 
				{
	...
	if (wp->Flags&WINDOWDRAG)
		{
	/*	SetDrMd(wind->RPort,JAM1);
		SetAPen(wind->RPort,clrs->hitbox_selected);
...
if (wind->Flags&WINDOWACTIVE) {FLAG=0;clrs=wp->ActiveClrs;gclrs=&Acolors;}
else {FLAG=1;clrs=wp->InactiveClrs;gclrs=&Icolors;}

wp->Title=cs;
if (!(StudioBase->defaults.flags&WIND_SHORTTITLEBAR) || FLAG==0)
	{
	if (wp->Flags&WINDOWCLOSE) 	L=24;
	if (wp->Flags&WINDOWDEPTH)	R=R-24;
