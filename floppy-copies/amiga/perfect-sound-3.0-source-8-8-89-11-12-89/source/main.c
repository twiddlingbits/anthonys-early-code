...
if (click && gad_sel(DEL_RNG_X,DEL_RNG_Y,mousex,mousey)) {     /* delete a range */
   ...
   if (end_mark>=samp[cur].length && start_mark==0) goto whole;
   msg(Window,"Working...");
...
if (menunum==1 && itemnum==6) {  /** Mark entire sample **/
   if (samp[cur].lmem!=0) {
      start_mark = 0;
...
if (menunum==3 && itemnum==0) {    /* change default recording speed */
   ...
   record_speed=i;
   sprintf(line,"New record speed set to %d.",i);
   msg(Window, line);
   }
