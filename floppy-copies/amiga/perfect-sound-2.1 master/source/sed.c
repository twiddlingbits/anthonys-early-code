...
static struct NewWindow NewWindow = {
   ...
   0,0,
   "Perfect Sound   Copyright 1986 SunRize Industries  Ver 2.0 ",
   0,0,
   0,0,0,0,
   WBENCHSCREEN
   };
...
if (menunum==2 && itemnum==3)  {  /* RECORD */
   ...
   samp.rate[i]=127+45*record_delay;
...
if (menunum==2 && itemnum==0) {    /* change default recording speed */
   i=getint(Window,127+45*record_delay,124,999,
            "Enter a new recording period:",440);
   record_delay=(i-127)/45;
   sprintf(line,"%d is as close as I can get.",127+45*record_delay);
   msg(Window,line);
   }
