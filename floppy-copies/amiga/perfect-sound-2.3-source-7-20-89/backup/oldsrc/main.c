...
if (class==RAWKEY) {    /** look for function key press. Plays octaves **/
   ...
   if (code == 80) {
      compress_4(Window, &samp, cur);
      msg(Window, "Did compress 4\n");
      goto ignore;
      }

   if (code == 81) {
      compress_6(Window, &samp, cur);
      msg(Window, "Did compress 6\n");
      goto ignore;
      }

   if (code == 82) {
      compress_all(Window, &samp, cur);
      msg(Window, "Did compress all\n");
      goto ignore;
      }
