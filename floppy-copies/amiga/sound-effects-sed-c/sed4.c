...
if (samp->type[current]==STEREO) len=samp->length[current]*2;
else len=samp->length[current];
if (samp->oneshot==0) len=len+4;  /* non instruments have a marker */
...
write(fp,&len,4);
if (samp->oneshot[current]==0)
   write(fp,&samp->type[current],4);  /* remember which channel */
...
if (k==FORM) {
   ...
   read(in,&samp->length[i],4);
   if (samp->oneshot[i]==0) {  /* I use 1st lword to mark type if not */
      samp->length[i]=samp->length[i]-4; /* an instrument file */
...
if (samp->type[i]==STEREO) {
   samp->rmem[i]=AllocMem(samp->length[i],0);
   if (samp->rmem[i]==0) {
      msg(Window,"File to large for available memory.");
