	...
	if (sfp) {
		for (i=0; i < SFT_NUM_REGIONS; i++) {
			if (sfp->tag.dregions[i].region_name[0]!=NULL) {
				sprintf(comprn,"%s.%s",sfp->samp_name,sfp->tag.dregions[i].region_name);
...
if (i) {
	...
	if (i==OL_MAX_PATHS)
		telluser("OpenList","Can't add any more paths.",0);
	}
}
...
if ((open_list_win->Width-30)/(AllListsCount()+1) < OL_LISTWID) {
	...
	WaitPort(open_list_win->UserPort);
	message=(struct IntuiMessage *)GetMsg(open_list_win->UserPort);
...
kprintf("NEWSIZE\n");
	...
	else {	/** Should hardly, if ever, happen **/
kprintf("message class %04x%04x\n",message->Class);
		PutMsg(open_list_win->UserPort, message);
		for (i=0; i < 15; i++)
			WaitTOF();
		}
	...
	if ((open_list_win->Width-30)/(AllListsCount()+1) < OL_LISTWID) {
		RefreshOLWin();
		telluser("Window to small. Resize it larger",0,0);
		return(FALSE);
		}
	}
return(TRUE);
...
for (i=0; i < cnt; i++) {
	...
	new_gadget_size(olpath[k].newbut.gad_ptr,
		...
		10+i*width+10,/* x2 */
		23);
	if (olpath[i].newlist.gad_ptr->flags&HIDE_GADGET) {
		show_gadget(olpath[i].newlist.gad_ptr);
		show_gadget(olpath[i].newbut.gad_ptr);
		}
	k++;
	}
