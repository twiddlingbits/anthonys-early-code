...
for (bit=0; bit < 9; bit++)
	{
	if (((mask&(1<<bit)) ^ (mask2&(1<<bit))) || oval==11)
		{
		if (mask & (1<<bit)) SetAPen(rp,Dcolor);
		else SetAPen(rp,StudioBase->defaults.colors.gadget_window.background);
