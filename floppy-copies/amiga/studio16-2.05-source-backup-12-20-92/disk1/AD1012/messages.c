...
/*||PLAY||*/
	...
	/***************************************/
		...
		key=PS->key;
		if(NULL==key) telluser("AD1012 Handler:","KEY IS NULL!!!",NULL);
...
/*||RECORD||*/
	...
	/***************************************/
		...
		if (key->handler == hand)
			...
			if (CD[3].PS->region.parms.flags & SI_TAP_OUTPUT) SendW(RecordOutOn);
		...
		if (CD[3].PS)
			...
			if (CD[3].PS->key->handler==key->handler)
				...
				if (fr) CD[3].PS->region.parms.smpte_sampling_rate=(sa*smpte_mode)/fr;
					...
					{
					if (CRecFrames!=-1) CD[3].PS->region.parms.smpte_sampling_rate=(CRecSamps*smpte_mode)/(TC2Frm(CRecTc,StudioBase->defaults.smpte_mode)-CRecFrames);
					else CD[3].PS->region.parms.smpte_sampling_rate=0;
					}
...
/* * */
	...
	/****************************************/
		...
		if (event->arg1==(int)hand)
			...
			else telluser("AD1012","Was sent a 0 sampling rate");
			}
		}
