...
ULONG RecSamps;
ULONG RecFrames;
int CRecSamps,CRecFrames,CRecTc;
int DrecSamps,DrecFrames,DrecTc;
...
/*||PLAY||*/
	...
	/***************************************/
		...
		if (key->handler==hand)
			...
			if (lasttc==event->arg2 && lasttc!=NULL) PlayFirst(x,1);
			else 
				{
				PlayFirst(x,0);
				lasttc=event->arg2;
...
TRigGErLOop:
		...
		CRecSamps=0;CRecTc=0;
		if (new!=0) CRecFrames=TC2Frm(new,StudioBase->defaults.smpte_mode);
		else CRecFrames=-1;
		DrecSamps=0;DrecTc=0;
		if (new!=0) DrecFrames=TC2Frm(new,StudioBase->defaults.smpte_mode);
		else DrecFrames=-1;
		Enable();
...
STopLOop:
		if (key->handler == hand)
			...
			if (CD[14].PS && key->flags&CK_CHAN_LEFT)
				...
				if (fr) CD[14].PS->region.parms.smpte_sampling_rate=(sa*smpte_mode)/fr;
				else 
					{
					if (CRecFrames!=-1) CD[14].PS->region.parms.smpte_sampling_rate=(CRecSamps*smpte_mode)/(TC2Frm(CRecTc,StudioBase->defaults.smpte_mode)-CRecFrames);
					else CD[14].PS->region.parms.smpte_sampling_rate=0;
					}
			...
			if (CD[15].PS && key->flags&CK_CHAN_RIGHT)
				...
				if (fr) CD[15].PS->region.parms.smpte_sampling_rate=(sa*smpte_mode)/fr;
				else 
					{
					if (DrecFrames!=-1) CD[15].PS->region.parms.smpte_sampling_rate=(DrecSamps*smpte_mode)/(TC2Frm(DrecTc,StudioBase->defaults.smpte_mode)-DrecFrames);
					else CD[15].PS->region.parms.smpte_sampling_rate=0;
					}
				BroadcastEventParms(se_info_channel_done, key->as, key, 0,0,0,0);
				CD[15].PS=NULL;
				key->as=NULL;
