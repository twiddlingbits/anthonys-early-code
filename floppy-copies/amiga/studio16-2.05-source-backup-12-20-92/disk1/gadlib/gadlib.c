...
/** !!!! SOME DAY CORRECT USAGE OF INITIAL SETING OF HIDE_GAD flag */
...
static struct Screen *default_screen=0;

struct Task *tc_gadlib;			/** gadlib task's tc **/

short lock_gadlib=0;				/** inc when shouldn't close gadlib */
...
short ShiftDown=FALSE;	/** shift currently depressed? **/
...
struct GadgetHeader *determine_hit_gadget();
struct Task *MyCreateProc();
void active_gadget();
void inhandler();
void delete_list();
void handle_lmb_button();
void handle_lmb_box();
void handle_lmb_slider();
void handle_lmb_list();
void handle_lmb_down();
void delete_gadget();
void delete_slider();
void delete_button();
void delete_box();
...
void new_droplist_size();
void new_looplist_size();
...
void l_key_func();
struct Screen *get_screen_ptr();
void drag_list_or_box();
void draw_dd();
void send_dragin_msg();
char *dd_text();
...
/*tc_gadlib=MyCreateProc("gadlibtask",4,main_gadlib,4000);*/
...
/** Wait for child task to finish cleanup before we return **/
...
if (mask&(1<<gadlib_sigbit3)) {
	FindMaxStackSize();			/*** FOR DEBUG, REMOVE!!!! ***/
	killself();
	Wait(0);
	}
...
do {
	while (read_ptr!=write_ptr) {
		...
		if (code==GE_RAWKEY) {
			keycode=event_buffer[read_ptr].code;
			if (keycode==0x60 || keycode==0x61)  /** Left & Right Shift keys **/
				ShiftDown=TRUE;
			...
			for (node=key_list.mlh_Head; node->mln_Succ; node = node->mln_Succ) {
				...
				if (kn->w==NULL || kn->w==window)
					(*kn->func)(kn,keycode,event_buffer[read_ptr].qualifier);
				}
			Permit();
			}
		else {	/** Not GE_RAWKEY **/
			if (window) {
				...
				if (code==GE_DELTAMOVE) { /*!!!!!!	  SIMPLIFY THIS expresion VVVV ***/
					...
					else { /** handle_lmb_gadget's deals with LBUTTONDOWN **/
						handle_lmb_down(window,x,y,event_buffer[read_ptr].secs,event_buffer[read_ptr].micro);
						}
					}
				}
			}
...
else if (ng->flags&GAD_BOX)
	ptr = (struct GadgetHeader *)create_box(w,ng);
...
else if (g->flags&GAD_BOX) refresh_box(g);
}
...
else if (g->flags&GAD_LOOPLIST) new_looplist_size(g, x1, y1, x2, y2);
else if (g->flags&GAD_DROPLIST) new_droplist_size(g, x1, y1, x2, y2);
else if (g->flags&GAD_BOX) new_box_size(g, x1, y1, x2, y2);
}
...
if (g->flags&GAD_LIST)
	...
	delete_looplist(g);
else if (g->flags&GAD_BOX)
	delete_box(g);
}

/**************************************************************/

void handle_lmb_down(window, x, y, secs, micro)
struct Window *window;
unsigned long secs, micro;
{
unsigned long flags;
struct GadgetHeader *g;

g=determine_hit_gadget(window, x, y);
if (g) {
	ObtainSemaphore(&baselock);
	flags=g->flags;

	if (flags&GAD_LIST) 
	  handle_lmb_list(g, x, y, secs, micro);
	else if (flags&GAD_BUTTON)
		handle_lmb_button(g, x, y, secs, micro);
	else if (flags&GAD_SLIDER)
		handle_lmb_slider(g, x, y, secs, micro); 
	else if (flags&GAD_BOX)
		handle_lmb_box(g, x, y, secs, micro); 
...
struct GadgetHeader *determine_hit_gadget(window, x, y)
struct Window *window;
{
struct MinNode *node;
unsigned long flags;
...
for (node=gad_list.mlh_Head; node->mln_Succ; node = node->mln_Succ) {
	if (window==((struct GadgetHeader *)node)->w) {
		...
		if (!(flags&HIDE_GADGET)) {
			if (flags&GAD_LIST) {
			  if (is_list_hit(node, x, y)) {
					ReleaseSemaphore(&baselock);
					return((struct GadgetHeader *)node);
					}
				}
			else if (flags&GAD_BUTTON) {
				if (is_button_hit(node, x, y)) {
					ReleaseSemaphore(&baselock);
					return((struct GadgetHeader *)node);
					}
				}
			else if (flags&GAD_BOX) {
				if (is_box_hit(node, x, y)) {
					ReleaseSemaphore(&baselock);
					return((struct GadgetHeader *)node);
					}
				}
			else if (flags&GAD_SLIDER) {
				if (is_slider_hit(node, x, y)) { 
					ReleaseSemaphore(&baselock);
					return((struct GadgetHeader *)node);
					}
				}
...
if (g->notifyme)
	send_msg(g->notifyme, g, GADMSG_ACTIVE, g->flags,0,0,0,0);
ReleaseSemaphore(&baselock);
}
...
/****************************************************************/
/** Calculate knob sizes assuming AUTO_KNOB.	 i.e., scale in   **/
/** the direction that we are FREE and maximize in the         **/
/** direction we are not free															**/
/****************************************************************/
...
if (k->show_size >= k->list_size) {
	...
	if (k->flags&FREE_VERT) {
		n1=diff;	/** this number can get real big **/
		n2=k->knob_y-k->box_y1;
		d1=k->box_y2-k->box_y1-k->knob_height+1;
		/*** Calc n1*n2/d1 with no overflows & no rounding errors ***/
		tmp=n1/d1;
		remainder=n1-tmp*d1;
		k->top_element=tmp*n2+(n2*remainder)/d1;
#if 0
	...
	if (k->flags&FREE_HORIZ) {
		n1=diff;			/* can be big */
		n2=k->knob_x-k->box_x1;
		d1=k->box_x2-k->box_x1-k->knob_width+1;
		/*** Calc n1*n2/d1 with no overflows & no rounding errors ***/
		tmp=n1/d1;
		remainder=n1-tmp*d1;
		k->top_element=tmp*n2+(n2*remainder)/d1;
...
void handle_lmb_slider(k, x, y, secs, micro)
struct slider *k;
int x,y;
unsigned long secs, micro;
{
...
if (hit_knob_up(k, x, y) || hit_knob_left(k, x, y))
	return(TRUE);
if (hit_knob_down(k, x, y) || hit_knob_right(k, x, y))
	return(TRUE);
return(FALSE);
}
...
/********************************************************************/
/** the following local global variables are used by knob dragging **/
/********************************************************************/
...
if (k->c->slider_knob.hitbox_selected != k->c->slider_knob.hitbox_unselected)
	draw_knob(k,k->c->slider_knob.hitbox_selected);	  /* knob can change color when being dragged */
...
if (code==GE_LBUTTONUP) {
	k->flags &= ~BUTN_HIT;		/** !!! SHOULD rename GAD_HIT or something!!! **/
	if (k->c->slider_knob.hitbox_selected != k->c->slider_knob.hitbox_unselected)
		draw_knob(k,k->c->slider_knob.hitbox_unselected);	 /* knob can change color when being dragged */
	selected_gadget=NULL;
	selected_gadget_func=NULL;
	ObtainSemaphore(&baselock);	
	if (k->notifyme)
		send_msg(k->notifyme, k, GADMSG_RELEASED, k->flags,0,0,0,0);
	ReleaseSemaphore(&baselock);
...
/**************************************************************/
static short dle_x, dle_y;
static short dle_oldx, dle_oldy;
/**************************************************************/

int is_list_hit(l, x, y)

struct list *l;
int x,y;
{
return(x>=l->box_x && x<= (l->box_x2-STD_SLIDER_WIDTH-STD_SLIDER_GAP) && y>=l->box_y && y<=l->box_y2);
}

/**************************************************************/

void handle_lmb_list(l, x, y, secs, micro)
struct list *l;
int x,y;
unsigned long secs, micro;
{
...
static unsigned long lsecs=0, lmicro=0;
...
if (select >= 0 && select < l->show_size) {
	...
	if (le) {
		...
		if (x > l->box_x && x < l->box_x+10+8*select_len) {
			handle_new_select(l, le, select,DoubleClick(lsecs,lmicro,secs,micro));
			if (l->lw) {	/** droplists vanish here **/
				if (l->flags&LIST_DRAGABLE) {
					selected_gadget=(struct GadgetHeader *)l;
					selected_gadget_func=drag_list_or_box;
					}
				}
			}
		}
...
void drag_list_or_box(from_g, code, newx, newy)
USHORT code;
int newy, newx;
struct GadgetHeader *from_g;
{
static short state=DLE_IDLE;
static appear_count=0;
struct Window *w;
struct Layer *WhichLayer();
struct Layer *layer;
struct GadgetHeader *to_g;
int select;
struct list_entry *le;
struct list *tol;
short x,y;
...
if (code==GE_LBUTTONUP) {
	...
	selected_gadget_func=NULL;
	if (state==DLE_DRAG) {
		draw_dd(from_g,dle_oldx,dle_oldy);		/** Erase **/
		UnLockGadLibRender();
		state=DLE_IDLE;
		layer=WhichLayer(&from_g->w->WScreen->LayerInfo,from_g->w->LeftEdge+newx, from_g->w->TopEdge+newy);
		if (layer) {
			w=(struct Window *)layer->Window;
			if (w) {
				x=from_g->w->LeftEdge+newx-w->LeftEdge;
				y=from_g->w->TopEdge+newy-w->TopEdge;
				to_g=determine_hit_gadget(w, x, y);
				if (to_g) {
					if (to_g->flags&GAD_LIST) {	/** Dropping into a list **/
						le=0;
						tol=(struct list *)to_g;
						select = (y-tol->box_y-2)/9;
						if (select >= 0 && select < tol->show_size) {
							select = select + tol->top_element;
							le = (struct list_entry *)get_list_entry_ptr(tol, select);
							}
						send_dragin_msg(to_g,from_g,le);
						}
					else if (to_g->flags&GAD_BOX) { /** drop into a box gadget **/
						send_dragin_msg(to_g,from_g,0);
						}
					}
				}
			}
...
else if (code==GE_DELTAMOVE) {
	switch(state) {
		case DLE_IDLE:
			appear_count+=(abs(newx)+abs(newy));
			if (appear_count > 10) {
				LockGadLibRender();	/** Locked for duration of drag **/
				dd_calcxy(from_g);	/** get new dle_x, dle_y **/
				draw_dd(from_g,dle_x,dle_y);
				dle_oldx=dle_x;
				dle_oldy=dle_y;
				state=DLE_DRAG;
				}
		...
		case DLE_DRAG:
			draw_dd(from_g,dle_oldx,dle_oldy);
			dd_calcxy(from_g);	/** get new dle_x, dle_y **/
			draw_dd(from_g,dle_x,dle_y);
			dle_oldx=dle_x;
			dle_oldy=dle_y;
...
len=strlen(dd_text(g));
dle_x=g->w->WScreen->MouseX - len*4;
dle_y=g->w->WScreen->MouseY + 3;
if (dle_x < 0)
	dle_x=0;
if (dle_x+len*8 >= g->w->WScreen->Width)
	dle_x=g->w->WScreen->Width-len*8;
if ((dle_y-8) < 0)
	dle_y=8;
if (dle_y > (g->w->WScreen->Height-2))
	dle_y=g->w->WScreen->Height-2;
}

/**************************************************************/

void send_dragin_msg(tog,fromg,le)
struct GadgetHeader *fromg;
struct GadgetHeader *tog;
struct list_entry *le;
{
if (tog->notifyme) {
	if (fromg->flags&GAD_LIST)
		send_msg(tog->notifyme, tog, GADMSG_DRAG_AND_DROP, tog->flags,((struct list *)fromg)->active_entry->text,fromg->user_data,fromg,le);
	else if (fromg->flags&GAD_BOX)
		send_msg(tog->notifyme, tog, GADMSG_DRAG_AND_DROP, tog->flags,((struct box *)fromg)->text,fromg->user_data,fromg,0);
	}
}
...
if (g->flags&GAD_LIST)
	str=((struct list *)g)->active_entry->text;
else if (g->flags&GAD_BOX)
	str=((struct box *)g)->text;
return(trim_text(g,str));
}
...
FUNCTION
	Selects a new entry in a list. For example, create_list uses:
		handle_new_select(l,get_list_entry_ptr(l, nl->initial_value),
								nl->initial_value);
	handle_new_select acts just like the user clicked an entry-
	the user_func() is called etc.
INPUTS
	l			The list to work on
	le			this list_entry is to be selected
	select	this is the index of the above le.	This info
				is redundant, but it speeds internal things up
				a little.
	allowdc	TRUE if a double click should be processed if
				set entry already set.
RESULTS
	NONE
##############################################################*/

void handle_new_select(l, le,	 select, allowdc)
...
if (le->flags&LE_SELECTED) {
	...
	if (l->flags&LIST_SINGLESELECT || !ShiftDown) {
...
if (nl->e_text) {		/** Add pre-initilized list entries **/
	...
	if (nl->initial_value < i && nl->initial_value!=-1) {
		handle_new_select(l,get_list_entry_ptr(l, nl->initial_value),nl->initial_value,FALSE);
		}
	}
...
if (l->flags&LIST_USEKEYS) {
	nkn.func=l_key_func;
	if (nl->flags&HIDE_GADGET)
		nkn.w=(struct Window *)-1;
	else
		nkn.w=l->lw;
...
if (code==0x4c) { /** up arrow **/
	if (l->current > 0) {
		handle_new_select(l,(le=get_list_entry_ptr(l, l->current-1)),l->current-1,FALSE);
		scroll_to_show(l, l->current);
		}
	}
	...
	if (l->current!=-1)
		handle_new_select(l,get_list_entry_ptr(l, l->current),l->current,FALSE);
	}
}
...
FUNCTION
	If needed, scroll's the passed list so that the passed
	list entry is visable.  Updates slider also to reflect
	new list position.
INPUTS
	l			The list to work on
	li			list_index. You can use "get_list_entry_index(l, le)"
				to find, if needed.
RESULTS
	TRUE if a scroll occured, else FALSE
##############################################################*/

int scroll_to_show(l, li)
struct list *l;
long li;
{
/*void *tempfunc;*/
...
if (li < l->top_element) {
...
#endif
	l->slider->top_element=li;
	move_knob(l->slider);
	return(TRUE);
	}

else if (li >= (l->top_element+l->show_size)) {
#if 0
	l->top_element=li-l->show_size+1;
	refresh_list_text(l);
	l->slider->top_element=l->top_element;
	calc_knob_xy(l->slider);
	tempfunc=l->slider->func;
	...
	l->slider->func=tempfunc;
#endif
	l->slider->top_element=li-l->show_size+1;
	move_knob(l->slider);
	return(TRUE);
	}
else {
...
if (!(l->flags&HIDE_GADGET)) {
	...
	if (l->flags&GAD_SIBLING)	/** must be droplist!!!! AGHHH NEED BETTER WAY **/
...
for (node=top_node; node->mln_Succ; node = node->mln_Succ) {
	...
	if (i++==l->top_element)
		top_node=node;
	if ((len=my_strlen(trim_text(l,le->text)))>max_size)
		max_size = len;
	}
...
for (node=top_node; node->mln_Succ; node = node->mln_Succ) {
	...
	if (le->flags&LE_HIGHLIGHT) 		/** Optional highlight **/
		SetAPen(rp, le->highlight_color);
...
/*############################################################
NAME	
	highlight_list_entry

FUNCTION
	Renders the indicated list entry in the indicated color.
	Also srolls the list if needed to view list entry.
INPUTS
	l			The list to work on
	le			Make sure this list entry is display in c
	c			color to display list entry in
RESULTS
	NONE
##############################################################*/

void highlight_list_entry(l, le, c)
struct list *l;
struct list_entry *le;
unsigned char c;
{
...
le->flags|=LE_HIGHLIGHT;
le->highlight_color=c;
li=get_list_entry_index(l, le);
if (!scroll_to_show(l, li))
	refresh_list_text(l);
}

/*############################################################
NAME	
	unhighlight_list_entry

FUNCTION
	Renders the indicated list entry in its normal, non
	highlighted color.  Does not scroll the list.
INPUTS
	l			The list to work on
	le			Make sure this list entry is display in c
RESULTS
...
void unhighlight_list_entry(l, le)
struct list *l;
struct list_entry *le;
{
le->flags&=(~LE_HIGHLIGHT);
...
while(absdelta--) {
	...
	len=my_strlen(trim_text(l,le->text));
	if (len > l->max_show_chars)
		len = l->max_show_chars;
	Text(rp, trim_text(l,le->text), len);
	if (le->flags&LE_SELECTED) {
		if (l->flags&GAD_SIBLING) {
			SetAPen(rp, l->c->droplist.draw_unselected);
			SetBPen(rp, l->c->droplist.hitbox_unselected);
...
NAME	
	delete_list_entry -
FUNCTION
	removes an entry from struct *list.
INPUTS
...
FUNCTION
	...
	that matches the passed string.	Currently case insensitive
INPUTS
	list, str
RESULTS
...
FUNCTION
	Given a pointer to a list_entry, this will delete the entry.
INPUTS
	struct list *
	struct list_entry *
RESULTS
...
FUNCTION
	Given an index, this will delete the appropriate list entry.
	For example, an index of 1 would delete the second entry
	in the list of list_entrys
INPUTS
	list *
	int 
RESULTS
...
FUNCTION
	Scan's the list of list_entrys and returns a pointer
	to entry "i"	
INPUTS
	struct list *l
	int i
...
FUNCTION
	Scan's a list looking for the entry who's text matches the
	passed string.
INPUTS
	list *
	char *
...
FUNCTION
	Scan's a list and finds the first entry who's user_data
	matches
...
if (le) {
	handle_new_select(l, le, get_list_entry_index(l,le),FALSE);
	}
}
...
void handle_lmb_button(b, x, y, secs, micro)
struct button *b;
int x,y;
unsigned long secs, micro;
{
...
/** (and reselect when the mouse moves back over gadget) **/
	...
	if (code==GE_DELTAMOVE) {
		...
		if (b->flags&BUTN_HIT) {
			if (!is_button_hit(b, butnxpos, butnypos)) {
				xor_button(b);
				}
			}
		else {		/** !BUTN_HIT **/				
...
NAME
	xor_button - 
FUNCTION
	Toggles that state of BUTN_HIT as well as a button's imagery
INPUTS
...
nl.flags = LIST_COOL|HIDE_GADGET|GAD_SIBLING|LIST_SINGLESELECT|(nd->flags&(KEEP_ONE_ACTIVE|LIST_SORTED));
...
/** since droplists are created hidden, they do not have a window **/
...
if (d->l->flags&HIDE_GADGET) {
	...
	w=QuickWindow(d->c, d->box_x1+d->dw->LeftEdge-2, d->box_y1+d->dw->TopEdge + STD_DROP_ARROW_HEIGHT + 1, /** WAS +2 **/
		d->box_x2+d->dw->LeftEdge+2, d->box_y2+d->dw->TopEdge + 2 /* WAS 3*/,0,BORDERLESS, 0, NULL);
	if (w==0) 
...
void new_droplist_size(d, x1, y1, x2, y2)
struct droplist *d;
short x1,x2;
short y1,y2;
{
...
d->box_x2=x2;
d->box_y2=y2;
new_button_size(d->b, x2 - STD_DROP_ARROW_WIDTH + 1, y1, x2, y1 + STD_DROP_ARROW_HEIGHT - 1);
new_list_size(d->l, 2, 0, x2 - x1 + 2, y2 - y1 - STD_DROP_ARROW_HEIGHT);
}
...
if (le) {
	handle_new_select(ll->l, le, next,FALSE);
	}
...
void new_looplist_size(ll, x1, y1, x2, y2)
struct looplist *ll;
short x1,x2;
short y1,y2;
{
...
ll->box_x2=x2;
ll->box_y2=y2;
new_button_size(ll->b, x1, y1, x2, y2);
}
...
NewWindow.DetailPen = default_colors.gadget_window.draw_selected;
NewWindow.BlockPen = default_colors.gadget_window.draw_unselected;
...
if (screen)
	...
	NewWindow.Screen = get_screen_ptr();
w = (struct Window *)OpenWindow(&NewWindow);
if (w) {
	SetAPen(w->RPort, c->gadget_window.background);
...
if ((g->flags&GAD_SLIDER) && (g->flags&SLIDER_ARROWS)) {
	...
	if (((struct list *)g)->kn)
		((struct list *)g)->kn->w=(struct Window *)-1;		/** No key messages! **/
	}
else if (g->flags&GAD_DROPLIST) {
...
/*********************************************/
/********* box gadget functions  *************/
/*********************************************/

struct box *create_box(w,nb)
struct Window *w;
struct NewBox *nb;
{
struct box *b;

b=(struct box *)AllocMem(sizeof(struct box),MEMF_CLEAR);
if (b==0) return(0);

if (nb->text) 
	set_box_text(b,nb->text);
...
NAME	-
	refresh_box
FUNCTION
	refreshes a box's imagery.  Takes into account MANUAL_XOR flags
	and BUTN_HIT.
INPUTS
	b - box to refresh
RESULTS
...
NAME	-
	draw_box -
FUNCTION
	Renders a box and its associated text.
INPUTS
	b - box to render
RESULTS
	NONE
...
if (b->flags&BORDER_3D) {
	draw_indent(b->bw->RPort, b->box_x1, b->box_y1, b->box_x2, b->box_y2, top_col, bot_col);
	draw_indent(b->bw->RPort, b->box_x1+2, b->box_y1+1, b->box_x2-2, b->box_y2-1, bot_col, top_col);
	}

if (b->text) {
	t=trim_text(b,b->text);
	len = my_strlen(t);
	ypos = b->box_y1+(b->box_y2 - b->box_y1+6)/2;
	if (len*8 > (b->box_x2-b->box_x1-9))
		len = (b->box_x2-b->box_x1-9)/8;
	if (len < 0) len=0;
	xpos = b->box_x1+5;
	SetAPen(b->bw->RPort, apen);
	Move(b->bw->RPort, xpos, ypos);
	Text(b->bw->RPort, t, len);
	}
...
FUNCTION
	Resize or move a box gadget.	Imagery is not updated, just
	internal data structures.
INPUTS
	b	box to update
	x1,x2 new position,size
...
void new_box_size(b, x1, y1, x2, y2)
...
void handle_lmb_box(b, x, y, secs, micro)
struct box *b;
int x,y;
unsigned long secs, micro;
{
...
NAME	-
	set_box_text
FUNCTION
c	Changes a box's text.  The old text's memory is free'd
	and a copy is made of the new text.	  You must call
	refresh_gadget() to display the new text.
INPUTS
	b - box whos text is to change
	text - char * to text
RESULTS
	NONE
##############################################################*/

void set_box_text(b,text)
struct box *b;
char *text;
{
int len;
...
static struct NewButton cancelbutnguide = 
	{NULL,20,29,50,41,BUTN_COOL|MANUAL_XOR,0,0,&default_colors,0,0,0,"CANCEL"};
static struct NewButton okbutnguide = 
	{NULL,20,29,50,41,BUTN_COOL|MANUAL_XOR,0,0,&default_colors,0,0,0,"OK"};
static last_te=0;
static last_le=0;
char *l1,*l2;
unsigned long flags;
...
struct NewKeyNotice NewKey;
int width;
struct Window *w;
struct GadgetHeader *g;
struct GadMsg *msg;
struct key_notice *unkn;
int return_code;
struct TextFont *fnt;
int MX,MY;
...
/** The following code stops "creeping" **/
...
if (w=(struct Window *)OpenWindow(&NewWindow)) {
	...
	SetAPen(w->RPort,default_colors.gadget_window.background);
	...
	if (okbutn.notifyme) {
		if (g=create_gadget_chain(w, &okbutn)) {
			NewKey.w=w;
			NewKey.func=UserNoticeKeyFunc;
			NewKey.user_data=(APTR)&okbutn;
			unkn=create_key_notice(&NewKey);
			if (unkn) {
...
void UserNoticeKeyFunc(kn, code, qualifier)
struct key_notice *kn;
short code,qualifier;
{
struct NewButton *nb;

nb=(struct NewButton *)kn->user_data;
if (code==0x34 && (qualifier&0x40)) { /** Left-Amiga V? **/
	send_msg(nb->gad_ptr->notifyme, nb->gad_ptr, GADMSG_ACTIVE,0,0,0,0,0);
	}
else if (code==0x35 && (qualifier&0x40)) { /** Left-Amiga B? **/
	send_msg(nb->gad_ptr->notifyme, NULL, GADMSG_ACTIVE,0,0,0,0,0);
	}
}
...
RESULTS
	NONE
##############################################################*/
struct Isrvstr Inter;
unsigned short chip busy[] = {
	0x0000, 0x0000,
...
/*		  setmenus(screen, FALSE);
	...
	case PTR_NOTHING:
		...
		setall(screen,nothing, 16, 16,-1, 0);
		break;

	}	/** end of switch **/
if (mode==PTR_BUSY)
	{
	if (PFLAG==0) {PFLAG=1;AddTOF(&Inter,PointerFunc,NULL);}
	}
else
	{
	if (PFLAG==1) {PFLAG=0;RemTOF(&Inter);}
	}
}
...
if (cnt&1) return;
x=buzy[cnt>>1];
CopyMemQuick(x+10,busy+10,11*4);	/* 11 moves 30 times a second is worth the savings in chip ram */
}
...
void send_msg(port, g, type, flags,text,ud,from,le)
struct GadgetHeader *g;
struct MsgPort *port;
unsigned long flags,type;
char *text;
APTR ud;	
struct GadgetHeader *from;
struct list_entry *le;
{
struct GadMsg *msg;
...
NAME
	LockGadLibBase -
FUNCTION
	Obtains a Semaphore lock on the gadlib's internal structures.
...
NAME	
	UnLockGadLibBase() -
FUNCTION
	Frees the lock obtained by a call to LockGadLib().  Every
...
NAME
	LockGadLibRender -
FUNCTION
	Obtains a Semaphore lock on the gadlib's render lock.
...
NAME	
	UnLockGadLibRender() -
FUNCTION
	Frees the lock obtained by a call to LockGadLibRender().  Every
...
LockGadLibRender();
}	/** caused by conflicting base/renderlocks.	Fix better when have time! ***/
...
FUNCTION
	Sets the colors to be used by such functions as telluser,
...
FUNCTION
	Sets the screen to use for new_pointer(), telluser(), etc.
	If not called, IntuitionBase->ActiveScreen is the default.
INPUTS
	pointer to a intuition screen
RESULTS
	NONE
##############################################################*/

void set_default_screen(s)
struct Screen *s;
{
default_screen = s;
}
...
if (g->flags&TEXT_ONLY_FILE_NAME) {
	return(StrFileName(p));
	}
else if (g->flags&TEXT_ONLY_REGION_NAME) {
	return(StrRegionName(p));
	}
else {
	return(p);
	}
