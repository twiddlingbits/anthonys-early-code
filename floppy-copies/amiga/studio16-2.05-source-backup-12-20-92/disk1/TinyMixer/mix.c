...
if (wind->Width< (20+NumMix*16)) 
	...
	return;  /* MUST wait until resize to render */
	}
...
while (z<NumMix)
	{
	LOCKgad[z].box_y1=Y-10;
	LOCKgad[z].box_y2=Y-2;

	LOCKgad[z].box_x1=s+((w*z)/NumMix);
	LOCKgad[z].box_x2=s+((w*(z+1))/NumMix)-sp;

	LOCKgad[z].flags=LOCKgad[z].flags&~BUTN_HIT;
	if (LOCKgad[z].gad_ptr) LOCKgad[z].flags|=((struct button *)(LOCKgad[z].gad_ptr))->flags&BUTN_HIT;
	...
	LOCKgad[z].next=&LOCKgad[z+1];
	z++;
	}
MIXgad[z-1].next=NULL;
LOCKgad[z-1].next=NULL;
delete_gadget_chain(wind);

if (create_gadget_chain(wind,&MIXgad)==0)  {telluser("Tiny Mixer:","Could not create gadgets!",0); exit(10);}
if (create_gadget_chain(wind,&LOCKgad)==0)  {telluser("Tiny Mixer:","Could not create gadgets!",0); exit(10);}
...
if (FindTask(NULL)!=task) 
	...
	if (((struct button *)(LOCKgad[m]).gad_ptr)->flags & BUTN_HIT)
		{
		for(w=0;w<NumMix;w++) 
			{
			if (((struct button *)(LOCKgad[w]).gad_ptr)->flags & BUTN_HIT && w!=m && ((struct slider *)(MIXgad[w]).gad_ptr)->top_element!=k->top_element)
				{
/*				((struct slider *)(MIXgad[w]).gad_ptr)->top_element=k->top_element;*/
				if (Value>=0) BroadcastEventParms(se_cmd_set_channel_volume,Chan[w],Value,0,0,0);
				else BroadcastEventParms(se_cmd_set_channel_volume,Chan[w],0,0,0,0);
				}
			}
		}
	}
