...
char name[STUDIO16_MAX_PATH_SIZE];
...
/*************/
if(MyRead(fp,temp,11)!=11) return(CLE_EOF);
temp[11]=NULL;
stc=TCstr2int(temp);

if(1!=MyRead(fp,&a,1)) return(CLE_READ_ERROR);
while (a!=',' && a!=10) if(1!=MyRead(fp,&a,1)) return(CLE_READ_ERROR);
if (a==10) return(CLE_UNEXPECTED_EOL);
/*************/
if(3!=MyRead(fp,temp,3)) return(CLE_READ_ERROR);
temp[3]=NULL;
vol=DBstr2int(temp);
...
if(1!=MyRead(fp,&name[x],1)) return(CLE_READ_ERROR);
while (name[x]==' ') 
	if(1!=MyRead(fp,&name[x],1)) return(CLE_READ_ERROR);
...
while(name[x]!=',' && name[x]!=10)
	...
	if(1!=MyRead(fp,&name[x],1)) return(CLE_EOF);
	}
...
c=&ce->Rem[0];
x=0;f=0;
if (1!=MyRead(fp,&ce->Rem[x],1)) return(CLE_EOF);
while(ce->Rem[x]!=10)
	{
	if (ce->Rem[x]==' ') 
		{
...
if (len<num) 
	{
	rtn=stccpy(str,&bufr[1024-len],len);
	len=len-rtn;
	kprintf("'%s'",str);
	return(rtn);
	}
