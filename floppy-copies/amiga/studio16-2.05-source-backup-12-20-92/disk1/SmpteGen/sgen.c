...
if ((mod=(struct Module *)AddModule(&ModData))==0) 
	...
	telluser("SMPTE Gen:","Could Not Add Module",0);
	closelibraries();
	exit(10);
	}
...
if (wind = OpenNewWind(&SMON_NewWindow,mod->state,NULL,NULL,NULL))
	...
	but[4].box_x2=((wind->Width-11-110)>>1)+110-3;
	but[5].box_x1=((wind->Width-11-110)>>1)+110+3;
	but[5].box_x2=wind->Width-11;

	x=0;
	DisplaySmpte(&x);
	create_gadget_chain(wind,&but[5]);
	WindGadSize();
...
if (wind)
	...
	for(i=0;i<NUMGADS;i++)
		...
		new_gadget_size(but[4].gad_ptr,110,16,((wind->Width-11-110)>>1)+110-3,26);
		refresh_gadget(but[4].gad_ptr);

		new_gadget_size(but[5].gad_ptr,((wind->Width-11-110)>>1)+110+3,16,wind->Width-11,26);
		refresh_gadget(but[5].gad_ptr);
