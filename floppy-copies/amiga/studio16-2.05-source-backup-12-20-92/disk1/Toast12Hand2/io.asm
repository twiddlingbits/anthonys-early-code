...
waitE2:	btst.b	#0,(a0)			*
	bne.s	waitE2			* Send RecDataReq

	move.w	#$000d,(a1)		*

waitE3:	btst.b	#0,(a0)			* DO NOT REMOVE ME, I prevent an ad1012
	bne.s	waitE3			* Interupt from being interpreted as Record Data!!

	move.l	#$1fff,d1		* BLOCKWORDS -1

waitERec btst.b	#1,(a0)
	bne.s	waitERec
	move.w	(a1),d0
	dbra	d1,waitERec
...
NotOn0	move.w	d1,(a1)+		* Write PlayCount[0]
	...
	bra	Sig
*-----------------------------------------------------------------------
Buff2	move.w	(a1)+,d0		* a1=Sound On
	move.w	(a1)+,d1		* Read PlayCount[0]
