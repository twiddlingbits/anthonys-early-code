...
struct Menu BlankProj[] = 
		{
		{ NULL, 3, 0, 8*10, 0, MENUENABLED, " Project ", NULL/*&Items[0]*/ },
		{ NULL,8*10+1, 0,15*8, 0, MENUENABLED, " Applications ", NULL },
		};
void MakeMenuesFit();
...
nwind->DetailPen=StudioBase->defaults.colors.gadget_window.bottomright_3d;
nwind->BlockPen=StudioBase->defaults.colors.gadget_window.topleft_3d;
...
nwind->Flags|=ACTIVATE|NOCAREREFRESH|BORDERLESS;

if (!(x&IL_NO_MENUS))
	nwind->IDCMPFlags|=ACTIVEWINDOW|INACTIVEWINDOW|MENUPICK|RAWKEY;
else
	nwind->IDCMPFlags|=ACTIVEWINDOW|INACTIVEWINDOW|RAWKEY|RMBTRAP;
...
if (!(x&IL_NO_MENUS))
	...
	if (menu) MakeMenuesFit(menu);
	wp->Project[0]=BlankProj[0];
	wp->Project[1]=BlankProj[1];
	wp->Project[1].NextMenu=menu;
	wp->Project[0].NextMenu=&wp->Project[1];
	ChangeMenuColor(&wp->Project[0]);

	SetMenuStrip(wind,&wp->Project);
	}
...
void FreeProjectMenu();
void FreeApplicationMenu();
extern struct Window *owind;
if(GlobActivWind==wind) GlobActivWind=NULL;

if (wind)
	{
	if (wind==owind) owind=NULL;
	if (wp->state!=NULL)
		{
	...
	if (wp->Flags&WINDOWCLOSE) 
		...
		if (StudioBase->defaults.flags&WIND_SHORTTITLEBAR && FLAG==1 && !(wp->Flags2&IL_NOSIMPLE))
			...
			if (wp->CloseButton.gad_ptr->flags&BUTN_HIT) 
				{
				draw_indent(wind->RPort,7,3,16,7,clrs.bottomright_3d^0xFF,clrs.topleft_3d^0xFF);
				}
			else
				{
				draw_indent(wind->RPort,7,3,16,7,clrs.bottomright_3d,clrs.topleft_3d);
				}
			}
		}
...
w=wind->Width; if (w<wind->MinWidth) w=wind->MinWidth;
h=wind->Height;if (h<wind->MinHeight) h=wind->MinHeight;

if (wp->W<wind->MinWidth) wp->W=wind->MinWidth;
if (wp->H<wind->MinHeight) wp->H=wind->MinHeight;
...
if (wp->Flags2 & HIDEMESSAGE)
	...
	else {telluser("Interfacelib","Alloc failed!!",NULL);}
	DeletePort(tempport);
	}
}
/*############################################################
