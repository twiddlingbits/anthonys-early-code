...
struct FileReqGlobes
	...
	long sigbit;
	struct Task *task;
	UBYTE PathSt[STUDIO16_MAX_PATH_SIZE];
	UBYTE UNDOBUFFER[STUDIO16_MAX_PATH_SIZE];
	UBYTE FileSt[51];
	UBYTE UNDOBUFFER2[51];
...
/**********************************************************/
if(!(frg->flags&FR_PATH_ONLY)) strcpy(frg->FileSt,name);
...
if (frg->flag==1) {HideWind(frg);return(NULL);}
	...
	{
	if(!(frg->flags&FR_PATH_ONLY)) strcpy(name,frg->FileSt);
...
while ((message=(struct IntuiMessage *)GetMsg(frg->wind->UserPort))!=0)
	...
	if (class == GADGETUP)
		...
		if(!(frg->flags&FR_PATH_ONLY))
			...
			if (gad==&frg->PathStGad) {ActivateGadget(&frg->FileStGad,frg->wind,NULL);Signal(frg->task,(1<<frg->sigbit));}
			}
		else Signal(frg->task,(1<<frg->sigbit));
		}
	}
}
/**************************************************************/
...
if (frg->wind = OpenNewWind(&NW,NULL,NULL,NULL))
	...
	if(!(frg->flags&FR_PATH_ONLY)) frg->FILElist.box_y2=frg->wind->Height-47;
	else frg->FILElist.box_y2=frg->wind->Height-47+12;

	frg->PATHlist.box_x1=((frg->wind->Width-30)*3)/4+4;
	frg->PATHlist.box_x2=frg->wind->Width-11;
	if(!(frg->flags&FR_PATH_ONLY)) frg->PATHlist.box_y2=frg->wind->Height-47;
	else frg->PATHlist.box_y2=frg->wind->Height-47+12;
	...
	if(!(frg->flags&FR_PATH_ONLY))	AddGList(frg->wind,&frg->FileStGad,NULL,1,NULL);
	AddGList(frg->wind,&frg->PathStGad,NULL,1,NULL);
...
/*	WindGadSize(frg);*/
	...
	if(!(frg->flags&FR_PATH_ONLY)) frg->PathStGad.TopEdge=frg->wind->Height-41;
	else frg->PathStGad.TopEdge=frg->wind->Height-29;
...
if(g->flags & LIST_DOUBLESELECT && *g->active_entry->text!='(')
	{
	if(!(frg->flags&FR_PATH_ONLY)) {frg->flag=2;Signal(frg->task,(1<<frg->sigbit));}
	}
...
if (frg->wind)
	...
	if(!(frg->flags&FR_PATH_ONLY))
		...
		{
		new_gadget_size(frg->FILElist.gad_ptr,10,16,((frg->wind->Width-30)*3)/4-5,frg->wind->Height-47+12);
		new_gadget_size(frg->PATHlist.gad_ptr,((frg->wind->Width-30)*3)/4+4,16,frg->wind->Width-11,frg->wind->Height-47+12);
		}
	refresh_gadget(frg->FILElist.gad_ptr);
	refresh_gadget(frg->PATHlist.gad_ptr);
