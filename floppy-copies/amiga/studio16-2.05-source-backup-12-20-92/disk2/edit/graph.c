...
void RethinkZoom()
{
max_zoom = EI[longest_ei].sfp->tag.length/EI[longest_ei].draw_width+1;
...
SetAPen(&ei->rp,0); 
SetDrMd(&ei->rp,JAM1);
RectFill(&ei->rp,0,0,ei->draw_width-1,ei->graph_height-1);
...
if (ei->graph_buffer && zoom_scale >= ei->gb_scale && zoom_scale!=1) {
	...
	if (thisstate->es_flags&ES_FAST_GRAPH)		/** fast graph? **/
		kount=1;
	else
		kount=zoom_scale/ei->gb_scale-1;
	for (x=gs; x < ge; x++) {
		j=index/ei->gb_scale;
		ptr=&ei->graph_buffer[j];
		maxval=*ptr;
		for (i=0; i < kount; i++)
			if (*++ptr > maxval)
				maxval=*ptr;
		offset=ei->div_tab[maxval];
/*		offset=ei->div_tab[ei->graph_buffer[index/ei->gb_scale]]; */
	   Move (&ei->rp, x, y-offset);
  	   MyDraw (&ei->rp, x, y+offset);
	...
	if (range <= SMALLBUFSIZE) {
		...
		if (quickbuffer) {
			...
			for (x=gs; x < ge; x++) {
				MyDraw(&ei->rp, x, y+GMAP(ei,quickbuffer[index-indexstart]));
				index += zoom_scale;
				}
	...
	/** Do a seek and read one byte **/		
		...
		for (x=gs; x < ge; x++) {
			MyDraw(&ei->rp, x, y+GMAP(ei,SampleGetOne(ei->sfp,index)));
			index += zoom_scale;
		   }
		}
	...
	if (gs==0 && gs < ge)	{/** expose left? **/
		if (buffertosample(ei,ge) < ei->sfp->tag.length) {/** Connect last dot? **/
			MyDraw(&ei->rp, x, y+GMAP(ei,SampleGetOne(ei->sfp,buffertosample(ei,ge))));
			}
		}
	new_pointer(PTR_NORMAL);
	}
...
if (snatch_gs > ei->graph_start) {    /** scroll left **/
	ScrollRaster(&ei->rp, delta,0,0,0,ei->draw_width-1,ei->graph_height-1);
...
for (i=0; i < EI_MAX; i++)
	if (EI[i].flags&EI_IN_USE) {
		...
		DoBlit(&EI[i],x2+1,EI[i].draw_width-1,0x0c0);
...
y = ei->graph_height/2;
Move(&ei->rp, 0, y);
MyDraw(&ei->rp, ei->draw_width-1, y);
...
switch(thisstate->grid) {
	...
	case 1:				/** frames **/
		if (StudioBase->defaults.smpte_mode)
			...
			gridcount=(int)(((float)rate)/29.97);
		break;
	case 2:				/** seconds **/
		gridcount=rate;
		break;
...
switch(thisstate->grid) {
	case 1:				/** frames **/
		if (StudioBase->defaults.smpte_mode) {
			...
			if (ei->sfp->tag.parms.smpte_sampling_rate)
				frate=ei->sfp->tag.parms.smpte_sampling_rate;
			else
				frate=ei->sfp->tag.parms.rate;
			frate=frate/29.97;		/** frate now = samples/frame **/
			x=(int)((float)si/frate);
			x=(int)((float)x*frate);
			}
		break;
...
if (start==0) {
	...
	x = GRAPH_X;
	draw_count_val = ei->org_gb_size/ei->draw_width+1;
...
for (index=0; index < tomove; index += ei->gb_scale) {
	...
	if (draw_count==0) {
	   ...
	   Move (edit_win->RPort, x, y-offset);
  	   MyDraw (edit_win->RPort, x, y+offset);
		UnLockGadLibRender();
		x++;
		}
	}
...
dw=DRAWWIDTH;
ei->draw_width=dw;
InitBitMap(&ei->bm,3L,dw,ei->graph_height);	/** depth, width, height of bitmap **/
...
if (ei->bm.Planes[0]==NULL)
	return(OUT_OF_MEMORY);
ei->bm.Planes[1]=ei->bm.Planes[0];	/** allow ClipBlit to use mask 1->7 **/
ei->bm.Planes[2]=ei->bm.Planes[0];
InitRastPort(&ei->rp);
ei->rp.BitMap = &ei->bm;
ei->rp.Mask = 1;
return(NO_ERROR);
...
int gs;
{																		/** Round Up **/
ei->graph_width = min(ei->draw_width,(ei->sfp->tag.length-gs+zoom_scale-1)/zoom_scale);
if (ei->graph_width < 0)
	ei->graph_width=0;
}
...
if (x < 0 || x >= rp->BitMap->BytesPerRow*8) {
/*	sprintf(bugmsg,"MyDraw Overflow in x: %d",x);*/
/*	telluser("Edit Error.  Please contact SunRize tech support.",bugmsg,0);*/
	return(0);
	}
if (y < 0 || y >= rp->BitMap->Rows) {
/*	sprintf(bugmsg,"MyDraw Overflow in y: %d",y);*/
/*	telluser("Edit Error.  Please contact SunRize tech support.",bugmsg,0);*/
	return(0);
	}
return(Draw(rp,x,y));
