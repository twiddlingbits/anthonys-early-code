...
static int current_scale_fraction;
static int addval;
static int addsign;
...
while (blocklen--) {
	...
	mem[index] = val;
	index++;
	current_scale_fraction+=addval;
	if (current_scale_fraction >= (1<<16)) {
		if (addsign > 0)
			current_scale+=(current_scale_fraction>>16);
		else
			current_scale-=(current_scale_fraction>>16);
		current_scale_fraction &= 0xFFFF;
		}
	}
}
...
while (len--) {
	...
	start_scale_fraction+=addval;
	if (start_scale_fraction >= (1<<16)) {
		if (addsign > 0)
			start_scale+=(start_scale_fraction>>16);
		else
			start_scale-=(start_scale_fraction>>16);
		start_scale_fraction &= 0xFFFF;
		}
	}
}
