...
write(fp,"ANNO",4);
i=68; write(fp,&i,4);
write(fp,"Recorded with PERFECT SOUND from SunRize Industries.  (408) 374-4962",
          68);

write(fp,"NAME",4);
i=strlen(samp->name);
if (i&1) {i++;k=1;}	/** pad odd lenght with zero **/
else {k=0;}
write(fp,&i,4);
write(fp,samp->name,i-k);
if (k==1) {
	i=0;
	write(fp,&i,1);
	}
