...
extern SHORT clipping;   /*!!!! remove =0 */
...
for ( ; line != NULL && !checkcan(); line = line->next) {
   ...
   if (line->flags & TEXTL_HYPHEN)
       nc++;
   w = obj->w - linestate.right_margin - line->w;
   if (nc != 0) {
   ...
   if ((linestate.flags&TEXTS_RAGRIGHT)&&(linestate.flags&TEXTS_RAGLEFT)) {
      tc.x += linestate.left_margin+(w>>1);     /* center justify */
      tc.startx += linestate.left_margin+(w>>1);
   }
   else if (linestate.flags & TEXTS_RAGLEFT) {
      tc.x += linestate.left_margin+w;          /* right justify */
      tc.startx += linestate.left_margin+w;
   }
   else if (line->flags & TEXTL_PARSTART) {       /* left or fill justify */
      tc.x += linestate.left_margin+linestate.paragraph_indent;
      tc.startx += linestate.left_margin+linestate.paragraph_indent;
   }
   else {
      tc.x += linestate.left_margin;
      tc.startx += linestate.left_margin;
   }
