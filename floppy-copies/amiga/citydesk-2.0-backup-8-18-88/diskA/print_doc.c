...
static struct Gadget print = {
   &copies,   /* next gadget */
   20,108,   /* origin XY of hit box relative to window TopLeft */
...
static struct Gadget Stop = {
   ...
   108,108,   /* origin XY of hit box relative to window TopLeft */
   47,13,   /* hit box width and height */
   NULL,   /* gadget flags */
   RELVERIFY,   /* activation flags */
   BOOLGADGET,   /* gadget type flags */
...
static struct NewWindow NewWindowStructure1 = {
   212,39,   /* window XY origin relative to TopLeft of screen */
   175,140,   /* window width and height */
   ...
   NULL,   /* custom bitmap */
   175,140,   /* minimum width and height */
   175,140,   /* maximum width and height */
...
while (TRUE) {
   ...
   while( (message=GetMsg(window->UserPort))!=0) {
      ...
      if (class == GADGETUP && g==&end_page)
         stcd_i(end_pageSIBuff,&last_page);

      if (class == GADGETUP && g==&start_page) {
