...
/* if (argc > 1) {
   ...
   }  TEST TO FORCE INTO HIRES MONITOR MODE */
...
/*** The following code handles the case where the user has already 
   ...
   switch(mode) {
      ...
      case DOBOX:
         ...
         showstat(screen,get_box(pg,thiswindow,mousex,mousey,0));
         newpointer(screen, mode);
         goto toploop;
   ...
   switch (mode) {
      ...
      case REFLOWTEXT:
         ...
         texttoflow = to->macro->macrotext->start;
    ...
    /*     tempobj=*obj; */
      ...
      case EDITFORMAT:
         ...
         to=(struct text_object *)obj->first->data;
         if (mode==CHEAPEDIT) {
            mt = to->macro->macrotext;
            text = texted(screen,mt->start,0,0,0,640,100,&status);

printf("texted status = %d\n",status);
      ...
      case SIZE:
         ...
         newpointer(screen,BUSY);
         to=(struct text_object *)obj->data;
         texttoflow = to->macro->macrotext->start; /** in case resized to */
         flowobj=reflowchain(obj);                 /** nothing **/
         if (flowobj==NULL) {
            mode=SIZE;
            }
         else {
      ...
      case DUPOBJECT:
         ...
         if (obj->type==TEXT) {
            to=(struct text_object *)obj->first->data;
            i=strlen(to->macro->macrotext->start)+1;
            texttoflow=(char *)AllocMem(i,0);
            if (texttoflow==NULL) {
               showstat(screen,OUTOFMEM);
               goto toploop;
               }
            strcpy(texttoflow, to->macro->macrotext->start);
            mode=FLOW;
            flowobj=NULL;
            newpointer(screen,mode);
            goto toploop;
...
if (class==RAWKEY) {
   switch(code) {
      ...
      case 83:  /* f3 delete everything on clipboard */
#if GERMAN
         if (telluser2(screen,"Ausfügen alle Objekten","an clipboard?",TRUE)) {
#else
         if (telluser2(screen,"Delete all objects","on clipboard?",TRUE)) {
#endif

            node=page[CB].objects.lh_Head;
            while (node->ln_Succ) {
               obj=(struct object *)node;
               if (!deleteobject(obj)) {
                  printf("PGM_ERROR04\n");
            ...
            SetDrMd(clipboard->RPort,JAM1);
            SetAPen(clipboard->RPort,GRAY1);
            RectFill(clipboard->RPort,0,0,std_width-1,screensize-1);
            }
         break;
