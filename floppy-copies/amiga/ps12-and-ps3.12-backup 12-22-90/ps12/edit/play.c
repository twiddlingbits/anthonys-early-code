...
int blklen;
struct play_msg *msg;
struct play_msg cmdmsg;         /** !!!! THIS NEEDS TO BE MEMF_PUBLIC **/
                              /** TAKE OFF STACK **/
...
setmem(&cmdmsg, sizeof(struct play_msg), 0);
cmdmsg.message.mn_Node.ln_Type = NT_MESSAGE;
cmdmsg.message.mn_ReplyPort = parent_play_port;
...
send_msg(&cmdmsg, CMD_BEGIN_PLAYING, unit_num, samp->rate, 64, len, 0,0);
   ...
   if (type==MSG_READY_FOR_DATA) {
      ...
      while (message=(struct IntuiMessage *)GetMsg(Window->UserPort)) {
         ...
         if (class==MOUSEBUTTONS && code==SELECTDOWN) {
            ...
            send_msg(&cmdmsg, CMD_STOP_PLAYING, unit_num,0, 0, 0, 0, 0);
            while (msg=GetMsg(parent_play_port))  /** flush msgs **/
            ReplyMsg(msg);
            goto stopplay;
            }
         }
      send_msg(&cmdmsg, CMD_DATA, samp->blockl1, blklen, 0, 0, 0, last_block);
      }
   else {
