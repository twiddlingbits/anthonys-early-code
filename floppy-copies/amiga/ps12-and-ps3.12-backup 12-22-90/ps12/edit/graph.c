...
if (!(samp[cur].flags&SAMPLE_OPEN)) return (0);
...
for (x=GRAPH_X; x < GRAPH_X+graph_width; x++) {
   ...
   if (flags&GRAPH_RIGHT)
      Draw (edit_win->RPort, x, y+(get_right_sample1(&samp[cur],index+offset)*(graph_height/2))/32768);
   else
      Draw (edit_win->RPort, x, y+(get_left_sample1(&samp[cur],index+offset)*(graph_height/2))/32768);
   }
}
...
if (!(samp[cur].flags&SAMPLE_OPEN) || marked_samp!=cur) return (0);
...
for (x=LGRAPH_X; x < LGRAPH_X+num_samples*6; x=x+6) {
   if (index >=0 && index <len) {
      endy = y+(get_left_sample1(&samp[cur],index)*(LGRAPH_H/2))/32768;
      if (index==start_mark) {
...
for (x=RGRAPH_X; x < RGRAPH_X+num_samples*6; x=x+6) {
   if (index >=0 && index <len) {
      endy = y+(get_left_sample1(&samp[cur],index)*(RGRAPH_H/2))/32768;
      if (index==end_mark) {
