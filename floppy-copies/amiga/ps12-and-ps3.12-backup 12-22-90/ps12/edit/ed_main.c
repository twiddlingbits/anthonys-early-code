...
show_msg(edit_win,100);   /** Welcome to perfect sound **/
...
/* printf("code %d\n",code); */
   ...
   if (code==25) {
      play(edit_win,start_mark,end_mark-start_mark+1,&samp[cur]);
      goto ignore;
      }
...
if (class==MENUPICK && code==MENUNULL) {
   if (marked_samp==cur)
      play(edit_win,0,start_mark,&samp[cur]);
   goto ignore;
   }
...
if (menunum==0 && itemnum==0) {     /** New **/
   ...
   for(i=0;i<MAX_SAMPLES;i++)
      if (samp[i].flags&SAMPLE_OPEN) {
         if (k==FALSE) {
            if (askuser(get_msg_ptr(236),get_msg_ptr(237))==FALSE)   goto ignore;
            k=TRUE;
            }
         delete_sample(i);
         }
...
if (menunum==0 && itemnum==1) {      /** Open **/
   if (load_samp(edit_win,studioBase->defaults.samp_io_path)==-1)
      goto ignore;
   copy_samp_to_list(&open_list);      /** update open_list **/
...
if (menunum==0 && itemnum==2) {      /* save ... */
   ...
   switch (subnum) {
   case 0: save_IFF(edit_win,&samp[cur],buf,FALSE);
           break;
   case 1: if (marked_samp==cur)
              save_samp(edit_win,&samp[cur],buf,start_mark,end_mark);
           else
              save_samp(edit_win,&samp[cur],buf,0,0);
           break;
   case 2: save_IFF(edit_win,&samp[cur],buf,TRUE);
   }
#endif
   goto ignore;
   }
...
if (menunum==1 && itemnum==7) {  /** set playback rate **/
   if (!(samp[cur].flags&SAMPLE_OPEN)) goto ignore;
   samp[cur].rate=getint(edit_win,samp[cur].rate,1000,28000,get_msg_ptr(235),450);
   }
...
if (menunum==2 && itemnum==12) {       /** Resample to new sampling rate **/
   if (!(samp[cur].flags&SAMPLE_OPEN)) goto ignore;
   if ((i=getslot(edit_win))==-1) {
      show_msg(edit_win,146);
      goto ignore;
      }
   ...
   samp[i].rate=getint(edit_win,samp[cur].rate,4000,40000, get_msg_ptr(241),350);
...
/* sort */
   ...
   line[0]=0;
   getstr(edit_win,line,get_msg_ptr(229),400);
  ...
  samp[i].cycles=getint(edit_win,8,0,256,get_msg_ptr(231),256);
 ...
 /* sampperhicycle should be power of 2 for most programs */
   ...
   if (j==FALSE)
       show_msg(edit_win,152);
   else
      show_msg(edit_win,153);
...
if (menunum==3 && itemnum==0) {    /* change default recording speed */
   i=getint(edit_win,(vid_clock/10)/record_speed,4000,40000,
            get_msg_ptr(242),450);
...
if (menunum==3 && itemnum==2) {            /* monitor the digitizer */
   ...
   CloseScreen(ts);
   ActivateWindow(edit_win);
   goto ignore;
   }

if (menunum==3 && itemnum==3)  {  /* record */
#if DEMO
   msg(edit_win,"This function disabled for DEMO");
#else
   set_digi_type();     /** PS_3 or PS_2 ? **/
   if (digi_type==PS_2)
      i=15500;
   ...
   samplen=digitze (edit_win,p1,p2,record_speed,memory,k);
   CloseScreen(ts);
   ActivateWindow(edit_win);
   samplen=even(samplen);
   if (samplen==0) {
      show_msg(edit_win,165);
...
if (menunum==4 && itemnum==0) {   /** filter on or off **/
   if (do_on) {
      filter_on();
      do_on = FALSE;
      show_msg(edit_win,167);
      goto ignore;
      }
   else {
      filter_off();
      do_on = TRUE;
      show_msg(edit_win,168);
      goto ignore;
      }
   }
...
if (menunum==4 && itemnum==1) {   /** turn demos off **/
   if (rename("ps.title","no_ps.title")==0)
      show_msg(edit_win, 169);
   else
      show_msg(edit_win, 170);
   goto ignore;
   }

if (menunum==4 && itemnum==2) {   /** turn demos on **/
   if (rename("no_ps.title","ps.title")==0)
      show_msg(edit_win, 171);
   else
      show_msg(edit_win, 172);
   goto ignore;
   }
...
if (menunum==4 && itemnum==3) {   /** real time echo **/
   set_digi_type();     /** PS_3 or PS_2 ? **/
   if (digi_type!=PS_3) {
      show_msg(edit_win, 248);     /** only works with 3.0 **/
      goto ignore;
      }
   echo_buf_len=getint(edit_win,500,10,3000,get_msg_ptr(246),450);
...
if (menunum==4 && itemnum==4) {   /** real time delay **/
   ...
   echo_buf_len=getint(edit_win,500,10,3000,get_msg_ptr(247),450);
...
marker = ((mousex - GRAPH_X)*samp[cur].length) + (graph_width/2); /* .5 round */
...
SetDrMd(edit_win->RPort, COMPLEMENT);
if (inserting) {
   x1 = GRAPH_X + (insert_mark*graph_width)/samp[cur].length;
   x2 = GRAPH_X + (insert_mark*graph_width)/samp[cur].length;
   }
else {
   x1 = GRAPH_X + (start_mark*graph_width)/samp[cur].length;
   x2 = GRAPH_X + (end_mark*graph_width)/samp[cur].length;
   }
...
marker = ((mousex - GRAPH_X)*samp[cur].length)/graph_width;
marker = odd(marker);
...
i = mousex - GRAPH_X - mark*graph_width/samp[cur].length;
if (i < 0)
   i = -i;
if (i < 10)
   return (TRUE);
