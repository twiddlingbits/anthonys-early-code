...
#define NUMGADS 5
/**************************************************************/
struct SampleFilePointer *GetSfp();
void PreFunc();
		...
		if (g==SampleName.gad_ptr)
			...
			if (!ce || strcmp(ce->path,msg->text))
				...
				if (!FindRegion(msg->text,&regn))
					...
					if (LIST_EMPTY==FALSE) Updatefunc();
					else
						{
						FirstAddfunc();
						MenuGhost(FALSE);
...
if (GadStructCmp(ce))
	...
	ce->regn.end=0;
	ce->regn.start=0;
	ce->regn.samp_name[0]=0;
	ce->regn.region_name[0]=0;
	ce->regn.flags=0;
	Gad2Struct(ce);
	Struct2Line(ce);
...
void Gad2Struct();
void Struct2Line();
struct CueEntry *dce=(struct CueEntry *)CueL->active_entry->user_data;
ADDEntry(&dce->regn,"");
...
if (tc)
	...
	DumpSamples();
	y=CueL->list_size;
	cea=(struct CueEntry **)AllocMem((y+1)*4,NULL);
...
for(x=0;x<CueL->list_size;x++)
	...
	le=find_list_entry_by_index(CueL,x);
	ce=(struct CueEntry *)le->user_data;
	if (strlen(path)>strlen(ce->regn.samp_name))
		{
		if (!stricmp(&path[strlen(path)-strlen(ce->regn.samp_name)],ce->regn.samp_name))
...
/*		FindRegion(SG_SampName,&ce->regn);*/
		Gad2Struct(ce);
		Struct2Line(ce);
