...
static struct NewList sample_list = {
   &sample_droplist,70,50,250,150,LIST_COOL|HIDE_GADGET|LIST_SINGLESELECT,list_func,
...
create_gadget(window,&HELPbutton);
...
printf("end\n");
   ...
   Wait((1<<window->UserPort->mp_SigBit) | (1<<testport->mp_SigBit)); 
	...
	if (msg=(struct GadMsg *)GetMsg(testport)) {
