...
struct NewButton playwave = {
   &stop,0,0,0,0,BUTN_COOL|MANUAL_XOR,0,0,0,0,0,0,"Play All"};

struct NewSlider hscroll= {
    &playwave,0,0,0,0,AUTO_KNOB|SLIDER_COOL|FREE_HORIZ|MANUAL_XOR|KNOB_TODDISH|SLIDER_ARROWS,
...
okreq(str,nondest)
char *str;
int nondest;		/** TRUE means this is a non-destructive edit **/
{
if (nondest)
	if (!NonDestEditOkay())
		return(FALSE);
		...
		while(code!=MENUNULL) {
			...
			if (menunum==2) {			/** Editor (project) **/
				...
				switch (itemnum) {
					...
					case 10:	/* dest cut */
						ProcessEdit(PE_DEST,DoCutEdit,0,0);
						break;

					case 11:	/* dest copy */
						ProcessEdit(PE_DEST|PE_PARM2,DoCopyEdit,0,0);
						break;
					...
					case 13:	/* dest paste */
						if (subnum==0)
							ProcessEdit(PE_DEST|PE_PARM2,DoPasteEdit,PT_INSERT_START,0);
						else if (subnum==1)
							ProcessEdit(PE_DEST|PE_PARM2,DoPasteEdit,PT_INSERT_END,0);
						else
							ProcessEdit(PE_DEST|PE_PARM2,DoPasteEdit,PT_OVERWRITE,0);
						break;

					case 14:	/* dest keep */
						ProcessEdit(PE_DEST,DoKeepEdit,0,0);
						break;

					case 15:	/* dest Delete */
						ProcessEdit(PE_DEST,DoDeleteEdit,0,0);
						break;
...
if (okstr==NULL)
	...
	return(okreq(okstr,PE_NONDEST&flags));
}
...
edit_win->RPort->Mask = 0x01;
SetAPen(edit_win->RPort, 1);
...
y = freehand_ei->graph_y+freehand_ei->graph_height/2+
	 GMAP(freehand_ei,freehand_buf[mousex - GRAPH_X + 1]);
Draw(edit_win->RPort, mousex+1, y);
edit_win->RPort->Mask = 0xFF;
}
...
{
if (zoom_scale)
	return((index/zoom_scale)*zoom_scale);
else
	return(index);
}
...
if (timerport) {
	...
	timermsg->tr_time.tv_micro=50000;
...
if (edit_win->Width < 440)			/** too small to draw status? **/
	return;
...
PutUpdateText(GRAPH_X+DRAWWIDTH/2-strlen(str_sm)*8-8,y,str_sm);
PutUpdateText(GRAPH_X+DRAWWIDTH/2+8,y,str_em);
UnLockGadLibRender();
}
...
if (EI[longest_ei].sfp->tag.parms.smpte_sampling_rate==0)
	...
	rate=(int)EI[longest_ei].sfp->tag.parms.smpte_sampling_rate;
