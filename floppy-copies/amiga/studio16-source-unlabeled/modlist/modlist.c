...
static struct NewList disklist =
			{NULL,10,24,130,90,LIST_COOL|LIST_SINGLESELECT|LIST_SORTED,0,0,0,0,0,0,0,0,0};

static struct NewList residentlist =
			{&disklist,140,24,265,90,LIST_COOL|LIST_SINGLESELECT|LIST_SORTED,FuncActivate,0,0,0,0,0,0,0,0};
...
if (mod_list_win==0) { 	/**!!!! ERROR!!!!! **/
	printf("cant open win\n");
	exit(10);	
	}
...
UpdateStandardModState(thismod,mod_list_win);
...
/*		printf("NewSize! %d %d\n",mod_list_win->Width,mod_list_win->Height);*/
		new_gadget_size(disklist.gad_ptr,10,24,
			10+(mod_list_win->Width*42)/100, mod_list_win->Height-26);
		new_gadget_size(residentlist.gad_ptr,disklist.gad_ptr->box_x2+10,
			24, disklist.gad_ptr->box_x2*2, mod_list_win->Height-26);
		new_gadget_size(unloadbutn.gad_ptr,
			residentlist.gad_ptr->box_x1+5, mod_list_win->Height-21,
			residentlist.gad_ptr->box_x1+70, mod_list_win->Height-7);
		new_gadget_size(button.gad_ptr,
			15, mod_list_win->Height-21,
			55, mod_list_win->Height-7);
