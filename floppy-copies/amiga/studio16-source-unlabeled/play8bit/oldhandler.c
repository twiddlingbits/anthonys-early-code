...
/*** first handle any buffers that are done playing ***/
   ...
   if (sigbits&(1<<audio_dev_port->mp_SigBit)) {
      while (msg=GetMsg(audio_dev_port)) { /** get AudioDevice CMD_WRITE replys **/
			mask = (short)(((struct IORequest *)msg)->io_Unit);
			if (mask&1) un=0;
			if (mask&2) un=1;
			if (mask&4) un=2;
			if (mask&8) un=3;
   ...
   if (sigbits&(1<<thismod->notifyme->mp_SigBit)) 
	while (msg=(struct StudioEvent *)GetMsg(thismod->notifyme)) {
		if (msg->type==se_cmd_set_channel_volume) {
			...
			*(hardvol[j])=pc[j].vol;
			pc[j].IOBlks[0]->ioa_Volume = pc[j].vol;
			pc[j].IOBlks[1]->ioa_Volume = pc[j].vol;
			pc[j].IOBlks[2]->ioa_Volume = pc[j].vol;
			pc[j].IOBlks[3]->ioa_Volume = pc[j].vol;
         ReplyMsg(msg);
         }
         ...
         exit(0);
			...
			if (tps->key->handler==thishandler) {
      	   ...
      	   IOB.ioa_Request.io_Unit=(struct Unit *)(1<<un);
...
************/
	...
	pc[un].IOBlks[pc[un].bufindex]->ioa_Request.io_Unit=(struct Unit *)(1<<un);
...
for (un=0; un < NUMCHANS; un++) {
	for (k=0;k<NUMBUFFERS;k++) {
	   if ((pc[un].IOBlks[k]=(struct IOAudio *)AllocMem(sizeof(struct IOAudio),0))==0) {
   	   while(k-->0)
   	      FreeMem(pc[un].IOBlks[k],sizeof(struct IOAudio));
	      return(TRUE);
	      }
	   movmem(allocIOB,pc[un].IOBlks[k],sizeof(struct IOAudio));
	   }
	}
return(FALSE);
}
...
for (un=0; un < NUMCHANS; un++) {
	for (i=0;i<NUMBUFFERS;i++) {
	   FreeMem(pc[un].IOBlks[i],sizeof(struct IOAudio));   /* finished with this IOB */
		}
   }
}
