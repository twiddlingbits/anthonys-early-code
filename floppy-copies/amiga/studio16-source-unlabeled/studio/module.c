...
state = (struct StandardModState *)AllocMem(nm->state_size, MEMF_CLEAR);
if (state==0) {
	DeletePort(port);
	if (name)
	   FreeMem(name, strlen(name)+1);
...
new->notifyme = port;
new->state = state;
state->state_size = nm->state_size;
...
FreeMem(m->state, m->state->state_size);
