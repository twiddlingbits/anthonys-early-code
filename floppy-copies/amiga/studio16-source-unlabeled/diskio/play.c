...
void InitModDiskIO()
{
static struct NewModule mod_data ={"DiskIO",0,sizeof(struct StandardModState)};
...
/*printf("diskio: preload! Allocating bigblock\n");*/
...
/*printf("Diskio: QuickRead blklen=%d\n",blklen); */
...
/*printf("diskio: msg chan done! freeing bigblock\n");*/

ps = (struct PlayingSample *)msg->arg1;
TEMP--;
printf("FREE PS %x %d lock %x\n",ps,TEMP,ps->fp);
/*
for (i=0; i < MAX_READ_LOCKS; i++) {
	printf("lock %x, ",ps->playing_samp->rfp[i]); 
	}
printf("\n");
*/
