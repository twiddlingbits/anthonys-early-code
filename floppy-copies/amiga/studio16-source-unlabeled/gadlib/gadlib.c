...
do {
   while (read_ptr!=write_ptr) {
      ...
      if (code==GE_DELTAMOVE) {
         ...
         if (xsum || ysum) {
            handle_gadgets(window, GE_DELTAMOVE, xsum, ysum);
            xsum=ysum=0;
            }
...
/****************************************************************/
/** Calculate knob sizes assuming AUTOKNOB.  IE, scale in the  **/
...
l->box_x2 = nl->box_x2;
l->box_y2 = nl->box_y2;
l->show_size = (nl->box_y2 - nl->box_y1 - 4)/9; /** change to font height! */
l->lw = w;
l->id=nl->id;
l->c=nl->c;
...
l->box_x2 = x2;
l->box_y2 = y2;
l->show_size = (y2 - y1 - 4)/9; /** change to font height! */
...
ReleaseSemaphore(&baselock);
#if 0			/** Delete this if it hasn't caused any problems **/
l->active_entry=le;
...
static struct NewButton cancelbutnguide = 
	{NULL,20,30,50,45,BUTN_COOL,0,0,&tucolors,0,0,0,"CANCEL"};
...
if (w=(struct Window *)OpenWindow(&NewWindow)) {
   ...
   Text(w->RPort, l2, my_strlen(l2));
	...
	if (flags&NOTICE_CANCEL) {
		if (w->Width > 200) {
		   okbutn.box_x1=32;
		   cancelbutn.box_x1=w->Width - 107;
			}
		else {
		   okbutn.box_x1=8;
		   cancelbutn.box_x1=w->Width - 83;
			}
	   okbutn.box_x2=okbutn.box_x1+30;
	   cancelbutn.box_x2=cancelbutn.box_x1+75;
