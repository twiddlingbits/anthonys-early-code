...
if (x==BODY) {
   ...
   rowlen=((iff->w+15)>>4) << 1;
   plen=rowlen*iff->h;
   iff->image=(UBYTE *)AllocMem(plen*iff->numplanes,0);
   if (iff->image==0) {
   ...
   offset=0;
   for (i=0;i<iff->h;i++)
      for(k=0;k<iff->numplanes;k++) {
         if (compmethd==1) {
            z=uncomp(&offset,comp+offset,
                     (BYTE *)iff->image+rowlen*i+k*plen,rowlen);
            if (z!=0) printf("ERROR! compression fault.\n");
            }
         else {
            movmem(comp+offset,(BYTE *)iff->image+plen*k+rowlen*i,rowlen);
