...
void PrintMaze (HWND hwnd)
{
	...
	hDlgPrint = CreateDialog(hMainInstance, (LPCTSTR)"PrintDlgBox", hwnd, lpfnPrintDlgProc) ;
	lpfnAbortProc = MakeProcInstance ((FARPROC) AbortProc, hMainInstance) ;
...
void PrintCells(HWND hwnd, HDC hDC)
{
	...
	if (line_width < 1)
		line_width=1;
	cell_width=cell_width_in_pixels*line_width;
	cell_height=cell_height_in_pixels*line_width;

	/** now print each cell **/

	for (x=0; x < maze_width_in_cells; x++) {
		for (y=0; y < maze_height_in_cells; y++) {
			DrawCellWithHDC(hDC, x, y, line_width, cell_width, cell_height, color_mazebackground);
			}
		}
}
...
int CountCandidateStartCells(void) 
{
	...
	/* first count the outside frame, except for corners */
	for (x=1; x < maze_width_in_cells-1; x++)
	{
		if (!(cell[x][0]&CELL_DONE) && (cell[x][0]&(LINE_SOUTH|LINE_EAST|LINE_WEST)))
			k++;

		if (!(cell[x][maze_height_in_cells-1]&CELL_DONE) && (cell[x][maze_height_in_cells-1]&(LINE_NORTH|LINE_EAST|LINE_WEST)))
			k++;
	}

	for (y=1; y < maze_height_in_cells-1; y++)
	{
		if (!(cell[0][y]&CELL_DONE) && (cell[0][y]&(LINE_NORTH|LINE_SOUTH|LINE_EAST)))
			k++;

		if (!(cell[maze_width_in_cells-1][y]&CELL_DONE) && (cell[maze_width_in_cells-1][y]&(LINE_NORTH|LINE_SOUTH|LINE_WEST)))
			k++;
	}

	/* now count four corners */
	if (!(cell[0][0]&CELL_DONE) && (cell[0][0]&(LINE_SOUTH|LINE_EAST)))
		k++;

	if (!(cell[0][maze_height_in_cells-1]&CELL_DONE) && (cell[0][maze_height_in_cells-1]&(LINE_NORTH|LINE_EAST)))
		k++;

	if (!(cell[maze_width_in_cells-1][0]&CELL_DONE) && (cell[maze_width_in_cells-1][0]&(LINE_SOUTH|LINE_WEST)))
		k++;

	if (!(cell[maze_width_in_cells-1][maze_height_in_cells-1]&CELL_DONE) && (cell[maze_width_in_cells-1][maze_height_in_cells-1]&(LINE_NORTH|LINE_WEST)))
		k++;
	
	/* finally count all cells in the maze except for the border cells */
	for (x=1; x < maze_width_in_cells-1; x++) 
		for (y=1; y < maze_height_in_cells-1; y++) 
			if (!(cell[x][y]&CELL_DONE) && (cell[x][y]&0xF))
				k++; 
...
//	for (*x = maze_width_in_cells-1; *x>=0; (*x)--)
	for (*x=0; *x < maze_width_in_cells; (*x)++)
		for (*y=0; *y < maze_height_in_cells; (*y)++) 
			if (!(cell[*x][*y]&CELL_DONE) && cell[*x][*y])
				for (i=0; i < 4; i++)	
