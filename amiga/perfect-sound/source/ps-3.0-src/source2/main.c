...
int vid_clock=357954;         /** NTSC constant **/
...
if (click && gad_sel(SET_PLAYBACK_X,SET_PLAYBACK_Y,mousex,mousey)) {      /* enter new playback rate */
   ...
   samp[cur].rate=getint(Window,samp[cur].rate,1000,27000,get_msg_ptr(235),450);
   }
...
if (menunum==1 && itemnum==12) {       /** Resample to new sampling rate **/
   ...
   samp[i].rate=getint(Window,samp[cur].rate,4000,40000, get_msg_ptr(241),350);
...
/* sort */
   ...
   if (marked_samp==oct[0]) {
      ...
      show_msg(Window,150);
      samp[i].oneshot=samp[oct[0]].length-1;
      }

   samp[i].repeat=samp[oct[0]].length-samp[i].oneshot;
...
if (menunum==3 && itemnum==0) {    /* change default recording speed */
   i=getint(Window,vid_clock/record_speed,4000,40000,
            get_msg_ptr(242),450);
   record_speed=vid_clock/i;
   sprintf(line,get_msg_ptr(208),vid_clock/record_speed);
   msg(Window, line);
   }
...
if (menunum==3 && itemnum==3)  {  /* record */
   ...
   if (digi_type==PS_2)
      ...
      i=30000;
   if (subnum==0 && vid_clock/record_speed > i) {
   ...
   samp[i].rate=vid_clock/record_speed;
   line[0]=0;
   getstr(Window,line,get_msg_ptr(225),400);
