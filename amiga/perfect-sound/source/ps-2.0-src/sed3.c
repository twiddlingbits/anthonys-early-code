...
stp=(float)(samp->em[cur]-samp->sm[cur])/(float)530;
addx=1;
if (stp<1) {
   addx=1/stp;
   stp=1;
...
Move(Window->RPort,(int)x,y);
SetAPen(Window->RPort,WHITE);

x=x+addx;
for (s=(float)samp->sm[cur]+stp;s<(float)samp->em[cur];s=s+stp) {
   val=128+*(samp->lmem[cur]+(int)s);
   val=GBOTY-val/SCALE;
   x=x+addx;
   Draw(Window->RPort,(int)x,val);
   }
}
